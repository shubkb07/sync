<?php
/**
 * Error Protection API: Recovery_Mode_Key_Service class
 */
...
final class Recovery_Mode_Key_Service {
	...
	public function generate_recovery_mode_token() {
		return generate_password( 22, false );
	}
	 ...
	 * @since 5.2.0
	 *
	 * @global PasswordHash $hasher Portable PHP password hashing framework instance.
	 *
	...
	public function generate_and_store_recovery_mode_key( $token ) {
		...
		$key = generate_password( 22, false );
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		if ( ! isset( $records[ $token ] ) ) {
			return new Sync_Error( 'token_not_found', __( 'Recovery Mode not initialized.' ) );
		}
		...
		if ( ! is_array( $record ) || ! isset( $record['hashed_key'], $record['created_at'] ) ) {
			return new Sync_Error( 'invalid_recovery_key_format', __( 'Invalid recovery key format.' ) );
		}
		...
		if ( ! $hasher->CheckPassword( $key, $record['hashed_key'] ) ) {
			return new Sync_Error( 'hash_mismatch', __( 'Invalid recovery key.' ) );
		}

		if ( time() > $record['created_at'] + $ttl ) {
			return new Sync_Error( 'key_expired', __( 'Recovery key expired.' ) );
		}
