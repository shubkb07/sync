<?php
/**
 * Error Protection API: sync_Recovery_Mode class
 */
...
class sync_Recovery_Mode {
	 ...
	 *
	 * @since 5.2.0
	 * @var sync_Recovery_Mode_Cookie_Service
	 */
	 ...
	 *
	 * @since 5.2.0
	 * @var sync_Recovery_Mode_Key_Service
	 */
	private $key_service;
	 ...
	 *
	 * @since 5.2.0
	 * @var sync_Recovery_Mode_Link_Service
	 */
	 ...
	 *
	 * @since 5.2.0
	 * @var sync_Recovery_Mode_Email_Service
	 */
	...
	/**
	 * sync_Recovery_Mode constructor.
	 *
	 * @since 5.2.0
	 */
	public function __construct() {
		$this->cookie_service = new sync_Recovery_Mode_Cookie_Service();
		$this->key_service    = new sync_Recovery_Mode_Key_Service();
		$this->link_service   = new sync_Recovery_Mode_Link_Service( $this->cookie_service, $this->key_service );
		$this->email_service  = new sync_Recovery_Mode_Email_Service( $this->link_service );
	}
	...
	public function initialize() {
		...
		add_action( 'logout', array( $this, 'exit_recovery_mode' ) );
		add_action( 'login_form_' . self::EXIT_ACTION, array( $this, 'handle_exit_recovery_mode' ) );
		add_action( 'recovery_mode_clean_expired_keys', array( $this, 'clean_expired_keys' ) );

		if ( ! next_scheduled( 'recovery_mode_clean_expired_keys' ) && ! installing() ) {
			schedule_event( time(), 'daily', 'recovery_mode_clean_expired_keys' );
		}

		if ( defined( 'sync_RECOVERY_MODE_SESSION_ID' ) ) {
			$this->is_active  = true;
			$this->session_id = sync_RECOVERY_MODE_SESSION_ID;
	 ...
	 *
	 * This will not change after recovery mode has been initialized. {@see sync_Recovery_Mode::run()}.
	 *
	 * @since 5.2.0
	 *
	 ...
	 * @return true|sync_Error True if the error was handled and headers have already been sent.
	 *                       Or the request will exit to try and catch multiple errors at once.
	 *                       sync_Error if an error occurred preventing it from being handled.
	 */
	public function handle_error( array $error ) {
		...
		if ( ! $extension || $this->is_network_plugin( $extension ) ) {
			return new sync_Error( 'invalid_source', __( 'Error not caused by a plugin or theme.' ) );
		}

		if ( ! $this->is_active() ) {
			if ( ! is_protected_endpoint() ) {
				return new sync_Error( 'non_protected_endpoint', __( 'Error occurred on a non-protected endpoint.' ) );
			}

			if ( ! function_exists( 'generate_password' ) ) {
				require_once ABSPATH . syncINC . '/pluggable.php';
			}
		...
		if ( ! $this->store_error( $error ) ) {
			return new sync_Error( 'storage_error', __( 'Failed to store the error.' ) );
		}
	...
	public function handle_exit_recovery_mode() {
		...
		if ( ! isset( $_GET['_syncnonce'] ) || ! verify_nonce( $_GET['_syncnonce'], self::EXIT_ACTION ) ) {
			sync_die( __( 'Exit recovery mode link expired.' ), 403 );
		}

		if ( ! $this->exit_recovery_mode() ) {
			sync_die( __( 'Failed to exit recovery mode. Please try again later.' ) );
		}
	 ...
	 * @since 5.2.0
	 *
	 * @global array $theme_directories
	 *
	 * @param array $error Error details from `error_get_last()`.
	 * @return array|false {
	 *     Extension details.
	...
	protected function get_extension_for_error( $error ) {
		global $theme_directories;

		if ( ! isset( $error['file'] ) ) {
			return false;
		}

		if ( ! defined( 'sync_PLUGIN_DIR' ) ) {
			return false;
		}

		$error_file    = normalize_path( $error['file'] );
		$plugin_dir = normalize_path( sync_PLUGIN_DIR );

		if ( str_starts_with( $error_file, $plugin_dir ) ) {
			$path  = str_replace( $plugin_dir . '/', '', $error_file );
			$parts = explode( '/', $path );
		...
		foreach ( $theme_directories as $theme_directory ) {
			$theme_directory = normalize_path( $theme_directory );
	...
	protected function is_network_plugin( $extension ) {
		...
		$network_plugins = get_active_network_plugins();
	...
	protected function store_error( $error ) {
		...
		switch ( $extension['type'] ) {
			case 'plugin':
				return paused_plugins()->set( $extension['slug'], $error );
			case 'theme':
