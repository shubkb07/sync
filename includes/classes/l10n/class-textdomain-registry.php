<?php
/**
 * Locale API: Textdomain_Registry class.
 *
...
class Textdomain_Registry {
	...
	public function get_language_files_from_path( $path ) {
		...
		$cache_key = md5( $path );
		$files     = cache_get( $cache_key, 'translation_files' );
		...
		if ( false === $files ) {
			...
			cache_set( $cache_key, $files, 'translation_files', HOUR_IN_SECONDS );
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param Upgrader $upgrader   Unused. Upgrader instance. In other contexts this might be a
	...
	public function invalidate_mo_files_cache( $upgrader, $hook_extra ) {
		...
		$translation_types = array_unique( list_pluck( $hook_extra['translations'], 'type' ) );

		foreach ( $translation_types as $type ) {
			switch ( $type ) {
				case 'plugin':
					cache_delete( md5( LANG_DIR . '/plugins/' ), 'translation_files' );
					break;
				case 'theme':
					cache_delete( md5( LANG_DIR . '/themes/' ), 'translation_files' );
					break;
				default:
					cache_delete( md5( LANG_DIR . '/' ), 'translation_files' );
					break;
			}
		}
	}
