<?php
/**
 * Meta API: Metadata_Lazyloader class
 */

/**
 * Core class used for lazy-loading object metadata.
 *
 * When loading many objects of a given type, such as posts in a Query loop, it often makes
 ...
 *
 * Site uses the Metadata_Lazyloader class to queue objects for metadata cache priming. The class
 * then detects the relevant get_*_meta() function call, and queries the metadata of all queued objects.
 *
 * Do not access this class directly. Use the metadata_lazyloader() function.
 *
 * @since 4.5.0
 */
#[AllowDynamicProperties]
class Metadata_Lazyloader {
	/**
	 ...
	 * @return void|Sync_Error Sync_Error on failure.
	 */
	public function queue_objects( $object_type, $object_ids ) {
		if ( ! isset( $this->settings[ $object_type ] ) ) {
			return new Sync_Error( 'invalid_object_type', __( 'Invalid object type.' ) );
		}
		 ...
		 * @param Metadata_Lazyloader $lazyloader  The lazy-loader object.
		 */
	 ...
	 *
	 * @since 4.5.0
	 * @deprecated 6.3.0 Use Metadata_Lazyloader::lazyload_meta_callback() instead.
	 *
	...
	public function lazyload_term_meta( $check ) {
		_deprecated_function( __METHOD__, '6.3.0', 'Metadata_Lazyloader::lazyload_meta_callback' );
	 ...
	 * directly, from either inside or outside the `Query` object.
	 *
	 * @since 4.5.0
	 * @deprecated 6.3.0 Use Metadata_Lazyloader::lazyload_meta_callback() instead.
	 *
