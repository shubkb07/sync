 ...
 * Example:
 *
 *     $smilies = Token_Map::from_array( array(
 *         '8O' => '😯',
 *         ':(' => '🙁',
 *         ':)' => '🙂',
 *         ':?' => '😕',
 ...
 *      Token_Map::from_precomputed_table(
 *          array(
 ...
 *
 * Since the `Token_Map` is designed for relatively static lookups, it can be
 ...
 *
 * This can be done with `Token_Map::precomputed_php_source_table()`.
 *
 ...
 * Example:
 *
 *     $map = Token_Map::from_array( array( 'simple_smile:' => '🙂', 'sob:' => '😭', 'soba:' => '🍜' ) );
...
class Token_Map {
	 ...
	 * corresponds to the release will help people identify where an
	 ...
	 *
	 * @return Token_Map|null Token map, unless unable to create it.
	 */
	public static function from_array( $mappings, $key_length = 2 ) {
		 ...
		 */
		usort( $shorts, 'Token_Map::longest_first_then_alphabetical' );
	 ...
	 * Token_Map::precomputed_php_source_tag().
	 *
	 * @since 6.6.0
	 *
	 * @param array $state {
	 ...
	 * }
	 *
	 * @return Token_Map Map with precomputed data loaded.
	 */
	public static function from_precomputed_table( $state ) {
		...
		if ( ! $has_necessary_state ) {
			_doing_it_wrong(
				__METHOD__,
				__( 'Missing required inputs to pre-computed Token_Map.' ),
				'6.6.0'
			);
			return null;
		}
