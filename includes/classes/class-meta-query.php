 ...
 *
 * Meta_Query is a helper that allows primary query classes, such as Query and User_Query,
 *
...
class Meta_Query {
	 ...
	 *
	 * See Meta_Query::__construct() for information on meta query arguments.
	 *
	 * @since 3.2.0
	 * @var array
	 */
	...
	/**
	 * Database table that where the metadata's objects are stored (eg $db->users).
	 *
	 * @since 4.1.0
	 * @var string
	 */
	 ...
	 * @param string $primary_table     Database table where the object being filtered is stored (eg users).
	 * @param string $primary_id_column ID column for the filtered object in $primary_table.
	 * @param object $context           Optional. The main query object that corresponds to the type, for
	 *                                  example a `Query`, `User_Query`, or `Site_Query`.
	 *                                  Default null.
	 * @return string[]|false {
	 ...
	 * Generates SQL clauses to be appended to a main query.
	 *
	 * Called by the public Meta_Query::get_sql(), this method is abstracted
		...
		if ( false === $alias ) {
			...
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				if ( 'LIKE' === $meta_compare_key ) {
					$join .= $db->prepare( " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key LIKE %s )", '%' . $db->esc_like( $clause['key'] ) . '%' );
				} else {
					$join .= $db->prepare( " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key = %s )", $clause['key'] );
				}
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				switch ( $meta_compare_key ) {
					case '=':
					case 'EXISTS':
						$where = $db->prepare( "$alias.meta_key = %s", trim( $clause['key'] ) );
						break;
					case 'LIKE':
						$meta_compare_value = '%' . $db->esc_like( trim( $clause['key'] ) ) . '%';
						$where              = $db->prepare( "$alias.meta_key LIKE %s", $meta_compare_value );
						break;
					case 'IN':
						$meta_compare_string = "$alias.meta_key IN (" . substr( str_repeat( ',%s', count( $clause['key'] ) ), 1 ) . ')';
						$where               = $db->prepare( $meta_compare_string, $clause['key'] );
						break;
					case 'RLIKE':
					case 'REGEXP':
						...
						$where = $db->prepare( "$meta_key $operator $cast %s", trim( $clause['key'] ) );
						break;
					...
					case 'NOT LIKE':
						...
						$meta_compare_value = '%' . $db->esc_like( trim( $clause['key'] ) ) . '%';
						$where              = $db->prepare( $meta_compare_string, $meta_compare_value );
						break;
					case 'NOT IN':
		...
		if ( array_key_exists( 'value', $clause ) ) {
			...
			switch ( $meta_compare ) {
				...
				case 'NOT IN':
					...
					$where               = $db->prepare( $meta_compare_string, $meta_value );
					break;

				case 'BETWEEN':
				case 'NOT BETWEEN':
					$where = $db->prepare( '%s AND %s', $meta_value[0], $meta_value[1] );
					break;

				case 'LIKE':
				case 'NOT LIKE':
					$meta_value = '%' . $db->esc_like( $meta_value ) . '%';
					$where      = $db->prepare( '%s', $meta_value );
					break;
	 ...
	 * In the case of Meta_Query, this only applies to 'IN' clauses that are
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		 ...
		 * @param Meta_Query $query        Meta_Query object.
		 */
