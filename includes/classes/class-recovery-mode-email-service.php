<?php
/**
 * Error Protection API: Recovery_Mode_Email_Link class
 */
...
final class Recovery_Mode_Email_Service {
	 ...
	 *
	 * @since 5.2.0
	 * @var Recovery_Mode_Link_Service
	 */
	private $link_service;

	/**
	 * Recovery_Mode_Email_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param Recovery_Mode_Link_Service $link_service
	 */
	public function __construct( Recovery_Mode_Link_Service $link_service ) {
	 ...
	 * }
	 * @return true|Sync_Error True if email sent, Sync_Error otherwise.
	 */
	public function maybe_send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		if ( ! $last_sent || time() > $last_sent + $rate_limit ) {
			if ( ! update_option( self::RATE_LIMIT_OPTION, time() ) ) {
				return new Sync_Error( 'storage_error', __( 'Could not update the email last sent time.' ) );
			}
		...
		return new Sync_Error( 'email_sent_already', $err_message );
	}
	...
	private function send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		$blogname = specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );

		$switched_locale = switch_to_locale( get_locale() );

		if ( $extension ) {
			$cause   = $this->get_cause( $extension );
			$details = strip_all_tags( get_extension_error_description( $error ) );
...
Sync has a built-in feature that detects when a plugin or theme causes a fatal error on your site, and notifies you with this automated email.
###CAUSE###
First, visit your website (###SITEURL###) and check for any visible issues. Next, visit the page where the error was caught (###PAGEURL###) and check for any visible issues.

###SUPPORT###

If your site appears broken and you can\'t access your dashboard normally, Sync now has a special "recovery mode". This lets you safely login to your dashboard and investigate further.
...
###DETAILS###'
		...
		$sent = mail(
			$email['to'],
			specialchars_decode( sprintf( $email['subject'], $blogname ) ),
			$email['message'],
			$email['headers'],
			$email['attachments']
		);
	...
	private function get_cause( $extension ) {
		...
		if ( 'plugin' === $extension['type'] ) {
			...
			$cause = sprintf( __( 'In this case, Sync caught an error with one of your plugins, %s.' ), $name );
		} else {
			$theme = get_theme( $extension['slug'] );
			$name  = $theme->exists() ? $theme->display( 'Name' ) : $extension['slug'];

			/* translators: %s: Theme name. */
			$cause = sprintf( __( 'In this case, Sync caught an error with your theme, %s.' ), $name );
		}
