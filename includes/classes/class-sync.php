...
class Sync {
	...
	/**
	 * Query variables for setting up the Sync Site Query Loop.
	 *
	 * @since 2.0.0
	 * @var array
	 */
	...
	/**
	 * Parses the request to find the correct Sync Site query.
	 *
	 ...
	 *
	 * @global Rewrite $rewrite Sync Site rewrite component.
	 *
	...
	public function parse_request( $extra_query_vars = '' ) {
		 ...
		 * @param Sync           $sync               Current Sync Site environment instance.
		...
		if ( ! empty( $rewrite ) ) {
			...
			if ( ! empty( $pathinfo ) && ! preg_match( '|^.*' . $rewrite->index . '$|', $pathinfo ) ) {
				$requested_path = $pathinfo;
			} else {
				// If the request uri is the index, blank it out so that we don't try to match it against a rule.
				if ( $req_uri === $rewrite->index ) {
					$req_uri = '';
				}
		...
		foreach ( $this->public_query_vars as $var ) {
			...
			} elseif ( isset( $_GET[ $var ] ) && isset( $_POST[ $var ] )
				&& $_GET[ $var ] !== $_POST[ $var ]
			) {
				sync_die(
			...
			} elseif ( isset( $perma_query_vars[ $var ] ) ) {
				$this->query_vars[ $var ] = $perma_query_vars[ $var ];
			}
			...
			if ( ! empty( $this->query_vars[ $var ] ) ) {
				...
				if ( isset( $post_type_query_vars[ $var ] ) ) {
					$this->query_vars['post_type'] = $post_type_query_vars[ $var ];
					$this->query_vars['name']      = $this->query_vars[ $var ];
				}
			}
		}
		...
		$this->query_vars = resolve_numeric_slug_conflicts( $this->query_vars );
		 ...
		 * @since 2.1.0
		 *
		 * @param Sync $sync Current Sync Site environment instance (passed by reference).
		 */
	...
	public function send_headers() {
		...
		if ( is_user_logged_in() ) {
			$headers = array_merge( $headers, get_nocache_headers() );
		...
		if ( ! empty( $this->query_vars['error'] ) ) {
			...
			// We're showing a feed, so Sync is indeed the only thing that last changed.
			if ( ! empty( $this->query_vars['withcomments'] )
				...
				$last_modified_post    = mysql2date( $date_format, get_lastpostmodified( 'GMT' ), false );
				$last_modified_comment = mysql2date( $date_format, get_lastcommentmodified( 'GMT' ), false );

				if ( strtotime( $last_modified_post ) > strtotime( $last_modified_comment ) ) {
					$last_modified = $last_modified_post;
				} else {
					$last_modified = $last_modified_comment;
				}
			} else {
				$last_modified = mysql2date( $date_format, get_lastpostmodified( 'GMT' ), false );
			}

			if ( ! $last_modified ) {
				$last_modified = gmdate( $date_format );
			}
			...
			if ( isset( $_SERVER['HTTP_IF_NONE_MATCH'] ) ) {
				$client_etag = unslash( $_SERVER['HTTP_IF_NONE_MATCH'] );
			} else {
				$client_etag = '';
			}
		...
		if ( is_singular() ) {
			$post = isset( $query->post ) ? $query->post : null;
		 ...
		 * @param Sync       $sync      Current Sync Site environment instance.
		 */
		$headers = apply_filters( 'headers', $headers, $this );
	...
	public function build_query_string() {
		...
		foreach ( (array) array_keys( $this->query_vars ) as $var ) {
			if ( '' !== $this->query_vars[ $var ] ) {
				$this->query_string .= ( strlen( $this->query_string ) < 1 ) ? '' : '&';

				if ( ! is_scalar( $this->query_vars[ $var ] ) ) { // Discard non-scalars.
					continue;
				}

				$this->query_string .= $var . '=' . rawurlencode( $this->query_vars[ $var ] );
			}
		}
	...
	/**
	 * Set up the Sync Site Globals.
	 *
	 * The query_vars property will be extracted to the GLOBALS. So care should
	 * be taken when naming global variables that might interfere with the
	 * Sync Site environment.
	 *
	 * @since 2.0.0
	 *
	 * @global Query     $query     Sync Site Query object.
	...
	public function register_globals() {
		...
		$GLOBALS['post']         = isset( $query->post ) ? $query->post : null;
	 ...
	 * @since 2.0.0
	 *
	 * @global Query $the_query Sync Site Query object.
	 */
	public function query_posts() {
		global $the_query;
		$this->build_query_string();
		$the_query->query( $this->query_vars );
	}
	...
	public function handle_404() {
		 ...
		 * @param Query $query Sync Site Query object.
		 */
		if ( false !== apply_filters( 'pre_handle_404', false, $query ) ) {
			return;
		}
		...
		if ( is_admin() || is_robots() || is_favicon() ) {
			...
			if ( $query->is_posts_page && ! empty( $this->query_vars['page'] ) ) {
				$content_found = false;
			}
	...
	/**
	 * Sets up all of the variables required by the Sync Site environment.
	 *
	 * The action {@see 'sync'} has one parameter that references the Sync object. It
	...
	public function main( $query_args = '' ) {
		...
		/**
		 * Fires once the Sync Site environment has been set up.
		 *
		 * @since 2.1.0
		 *
		 * @param Sync $sync Current Sync Site environment instance (passed by reference).
		 */
		do_action_ref_array( 'sync', array( &$this ) );
	}
}
