<?php
/**
 * Error Protection API: Recovery_Mode_Link_Handler class
 */
...
class Recovery_Mode_Link_Service {
	 ...
	 *
	 * @since 5.2.0
	 * @var Recovery_Mode_Cookie_Service
	 */
	private $cookie_service;

	/**
	 * Recovery_Mode_Link_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param Recovery_Mode_Cookie_Service $cookie_service Service to handle setting the recovery mode cookie.
	 * @param Recovery_Mode_Key_Service    $key_service    Service to handle generating recovery mode keys.
	 */
	public function __construct( Recovery_Mode_Cookie_Service $cookie_service, Recovery_Mode_Key_Service $key_service ) {
	...
	/**
	 * Enters recovery mode when the user hits login with a valid recovery mode link.
	 *
	 * @since 5.2.0
	 *
	...
	public function handle_begin_link( $ttl ) {
		...
		$url = add_query_arg( 'action', self::LOGIN_ACTION_ENTERED, login_url() );
		redirect( $url );
		die;
	}
