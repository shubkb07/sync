 ...
 *
 * The default directory is public/plugins. To change the default directory
 * manually, define `PLUGIN_DIR` and `PLUGIN_URL` in `sync-config.php`.
 *
 * @access private
 * @since 3.1.0
 *
 ...
 * use the public/blog-deleted.php, blog-inactive.php and
...
function ms_site_check() {
	...
	if ( '1' === $blog->deleted ) {
		if ( file_exists( CONTENT_DIR . '/blog-deleted.php' ) ) {
			return CONTENT_DIR . '/blog-deleted.php';
		} else {
			sync_die( __( 'This site is no longer available.' ), '', array( 'response' => 410 ) );
		}
	}

	if ( '2' === $blog->deleted ) {
		if ( file_exists( CONTENT_DIR . '/blog-inactive.php' ) ) {
			return CONTENT_DIR . '/blog-inactive.php';
		} else {
	...
	if ( '1' === $blog->archived || '1' === $blog->spam ) {
		if ( file_exists( CONTENT_DIR . '/blog-suspended.php' ) ) {
			return CONTENT_DIR . '/blog-suspended.php';
		} else {
			sync_die( __( 'This site has been archived or suspended.' ), '', array( 'response' => 410 ) );
		}
	}
...
function get_site_by_path( $domain, $path, $segments = null ) {
	 ...
	 * @since 3.9.0
	 *
	 * @param int|null $segments The number of path segments to consider. Sync Site by default looks at
	 ...
	 * @since 3.9.0
	 *
	 * @param null|false|Site $site     Site value to return by path. Default null
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	// If the network is defined in sync-config.php, we can simply use that.
	if ( defined( 'DOMAIN_CURRENT_SITE' ) && defined( 'PATH_CURRENT_SITE' ) ) {
		 ...
		 */
		$current_site = cache_get( 'current_network', 'site-options' );
		if ( ! $current_site ) {
			// Are there even two networks installed?
			$networks = get_networks( array( 'number' => 2 ) );
			if ( count( $networks ) === 1 ) {
				$current_site = array_shift( $networks );
				cache_add( 'current_network', $current_site, 'site-options' );
	...
	if ( empty( $current_site ) ) {
		/** This action is documented in includes/functions/ms-settings.php */
		do_action( 'ms_network_not_found', $domain, $path );
	...
	if ( empty( $current_blog ) && installing() ) {
	...
	if ( empty( $current_blog ) ) {
		 ...
		 * @since 3.9.0
		 *
		 * @param Network $current_site The network that had been determined.
...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $db->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that your host&#8217;s database server is not running, Sync Site was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $db->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
	...
	$msg .= __( 'Please Debug issue to find solution.' );
	...
	sync_die( $msg, $title, array( 'response' => 500 ) );
}
