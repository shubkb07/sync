 ...
 *                                Passed to the `initialize_site` hook.
 * }
 * @return int|Sync_Error The new site's ID on success, or error object on failure.
 */
function insert_site( array $data ) {
	global $db;
	...
	$prepared_data = prepare_site_data( $data, $defaults );
	if ( is_error( $prepared_data ) ) {
		return $prepared_data;
	}

	if ( false === $db->insert( $db->blogs, $prepared_data ) ) {
		return new Sync_Error( 'db_insert_error', __( 'Could not insert site into the database.' ), $db->last_error );
	}
	...
	if ( ! $new_site ) {
		return new Sync_Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param Site $new_site New site object.
	 */
	do_action( 'insert_site', $new_site );
	 ...
	 */
	do_action( 'initialize_site', $new_site, $args );
	...
	if ( has_action( 'mu_new_blog' ) ) {
		 ...
		 *
		 * @since MU (3.0.0)
		 * @deprecated 5.1.0 Use {@see 'initialize_site'} instead.
		 *
 ...
 * @param array $data    Site data to update. See {@see insert_site()} for the list of supported keys.
 * @return int|Sync_Error The updated site's ID on success, or error object on failure.
 */
function update_site( $site_id, array $data ) {
	global $db;

	if ( empty( $site_id ) ) {
		return new Sync_Error( 'site_empty_id', __( 'Site ID must not be empty.' ) );
	}

	$old_site = get_site( $site_id );
	if ( ! $old_site ) {
		return new Sync_Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	$data = prepare_site_data( $data, $defaults, $old_site );
	if ( is_error( $data ) ) {
		return $data;
	}

	if ( false === $db->update( $db->blogs, $data, array( 'blog_id' => $old_site->id ) ) ) {
		return new Sync_Error( 'db_update_error', __( 'Could not update site in the database.' ), $db->last_error );
	}
	 ...
	 *
	 * @param Site $new_site New site object.
	 * @param Site $old_site Old site object.
	 */
	do_action( 'update_site', $new_site, $old_site );
 ...
 * @return Site|Sync_Error The deleted site object on success, or error object on failure.
 */
function delete_site( $site_id ) {
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `Sync_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param Sync_Error $errors   Error object to add validation errors to.
	 * @param Site  $old_site The site object to be deleted.
	 */
	do_action( 'validate_site_deletion', $errors, $old_site );
	 ...
	 * @since 5.1.0
	 *
	 * @param Site $old_site Deleted site object.
	 */
	do_action( 'uninitialize_site', $old_site );
	...
	if ( false === $db->delete( $db->blogs, array( 'blog_id' => $old_site->id ) ) ) {
		return new Sync_Error( 'db_delete_error', __( 'Could not delete site from the database.' ), $db->last_error );
	}
	 ...
	 * @param Site $old_site Deleted site object.
	 */
	do_action( 'delete_site', $old_site );
 ...
 * @since 6.3.0 Use lazyload_site_meta() for lazy-loading of site meta.
 *
...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $db->get_results( sprintf( "SELECT * FROM $db->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) );

		update_site_cache( $fresh_sites, false );
	}

	if ( $update_meta_cache ) {
		lazyload_site_meta( $ids );
	}
}
 ...
 */
function lazyload_site_meta( array $site_ids ) {
	if ( empty( $site_ids ) ) {
		return;
	}
	$lazyloader = metadata_lazyloader();
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	cache_add_multiple( $site_data, 'sites' );
	cache_add_multiple( $blog_details_data, 'blog-details' );
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', 'check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', 'check_site_meta_support_prefilter' );
	}
 ...
 *
 * @see Site_Query::parse_query()
 *
 * @param string|array $args Optional. Array or string of arguments. See Site_Query::__construct()
 *                           for information on accepted arguments. Default empty array.
 * @return array|int List of Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                   or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new Site_Query();
 ...
 *                               See {@see insert_site()} for the possibly included data.
 * @param array        $defaults Site data defaults to parse $data against.
 * @param Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|Sync_Error Site data ready for a database transaction, or Sync_Error in case a validation
 *                        error occurred.
 */
function prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see insert_site()} for the possibly included data.
	 */
	$data = apply_filters( 'normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `Sync_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param Sync_Error     $errors   Error object to add validation errors to.
	 * @param array        $data     Associative array of complete site data. See {@see insert_site()}
	 *                               for the included data.
	 * @param Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( 'validate_site_data', $errors, $data, $old_site );
 ...
 * @since 5.1.0
 *
 * @param Sync_Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see insert_site()}
 *                               for the included data.
 * @param Site|null $old_site The old site object if the data belongs to a site being updated,
 *                               or null if it is a new site being inserted.
 */
function validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = sync_checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
 ...
 *
 * @global db     $db     Sync Site database abstraction object.
 * @global Roles $roles Sync Site role management object.
 *
 * @param int|Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|Sync_Error True on success, or error object on failure.
 */
function initialize_site( $site_id, array $args = array() ) {
	global $db, $roles;
	...
	if ( ! $site ) {
		return new Sync_Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( is_site_initialized( $site ) ) {
		return new Sync_Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param Site    $site    Site that is being initialized.
	 * @param Network $network Network that the site belongs to.
	 */
	$args = apply_filters( 'initialize_site_args', $args, $site, $network );

	$orig_installing = installing();
	if ( ! $orig_installing ) {
		installing( true );
	}
 ...
 * @return true|Sync_Error True on success, or error object on failure.
 */
function uninitialize_site( $site_id ) {
	global $db;
	...
	if ( ! is_site_initialized( $site ) ) {
		return new Sync_Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
	 ...
	 * @since MU (3.0.0)
	 *
	 * @param string $basedir Uploads path without subdirectory. See {@see upload_dir()}.
	 * @param int    $site_id The site ID.
	 */
 ...
 */
function is_site_initialized( $site_id ) {
	global $db;
	 ...
	 */
	$pre = apply_filters( 'pre_is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	if ( get_current_blog_id() !== $site_id ) {
		$switch = true;
		remove_action( 'switch_blog', 'switch_roles_and_user', 1 );
	...
	if ( $switch ) {
		restore_current_blog();
		add_action( 'switch_blog', 'switch_roles_and_user', 1, 2 );
	}
 ...
 *
 * @param Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	cache_delete( $blog_id, 'sites' );
	cache_delete( $blog_id, 'site-details' );
	cache_delete( $blog_id, 'blog-details' );
	cache_delete( $blog_id . 'short', 'blog-details' );
	cache_delete( $domain_path_key, 'blog-lookup' );
	cache_delete( $domain_path_key, 'blog-id-cache' );
	cache_delete( $blog_id, 'blog_meta' );
 ...
 * @since 5.1.0
 *
 * @param Site      $new_site The site object that has been inserted, updated or deleted.
 * @param Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id !== $old_site->network_id ) {
		maybe_update_network_site_counts( $new_site->network_id );
		maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 * @since 5.1.0
 *
 * @param Site      $new_site The site object after the update.
 * @param Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new Site( new stdClass() );
	}
 ...
 *
 * @param Site $new_site The site object after the update.
 * @param Site $old_site The site object prior to the update.
 */
function maybe_clean_new_site_cache_on_update( $new_site, $old_site ) {
 ...
 *                          for compatibility reasons. Accepts '1' or '0'.
 */
function update_blog_public_option_on_site_update( $site_id, $is_public ) {

	// Bail if the site's database tables do not exist (yet).
	if ( ! is_site_initialized( $site_id ) ) {
		return;
	}
 ...
 * @since 5.1.0
 */
function cache_set_sites_last_changed() {
	cache_set_last_changed( 'sites' );
}
 ...
 */
function check_site_meta_support_prefilter( $check ) {
	if ( ! is_site_meta_supported() ) {
