 ...
 * @since 0.71
 *
 * @global array $wcockneyreplace Array of formatted entities for certain common phrases.
 ...
 */
function texturize( $text, $reset = false ) {
	global $wcockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		/**
		 * Filters whether to skip running texturize().
		 *
		 * Returning false from the filter will effectively short-circuit texturize()
		 * and return the original text passed to the function instead.
		 *
		 * The filter runs only once, the first time texturize() is called.
		 *
		 * @since 4.0.0
		 *
		 * @see texturize()
		 *
		 * @param bool $run_texturize Whether to short-circuit texturize().
		 */
		$run_texturize = apply_filters( 'run_texturize', $run_texturize );
		...
		if ( isset( $wcockneyreplace ) ) {
			$cockney        = array_keys( $wcockneyreplace );
			$cockneyreplace = array_values( $wcockneyreplace );
		} else {
			/*
	...
	$shortcode_regex  = $found_shortcodes ? _get_texturize_shortcode_regex( $tagnames ) : '';
	$regex            = _get_texturize_split_regex( $shortcode_regex );

	$textarr = preg_split( $regex, $text, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY );

	foreach ( $textarr as &$curl ) {
		// Only call _texturize_pushpop_element if $curl is a delimiter.
		$first = $curl[0];
		if ( '<' === $first ) {
			...
			if ( str_contains( $curl, "'" ) ) {
				...
				$curl = texturize_primes( $curl, "'", $prime, $open_sq_flag, $closing_single_quote );
				$curl = str_replace( $apos_flag, $apos, $curl );
				$curl = str_replace( $open_sq_flag, $opening_single_quote, $curl );
			}
			if ( str_contains( $curl, '"' ) ) {
				$curl = preg_replace( $dynamic_characters['quote'], $dynamic_replacements['quote'], $curl );
				$curl = texturize_primes( $curl, '"', $double_prime, $open_q_flag, $closing_quote );
 ...
 */
function texturize_primes( $haystack, $needle, $prime, $open_quote, $close_quote ) {
	$spaces           = spaces_regexp();
	$flag             = '<!--prime-or-quote-->';
 ...
 */
function _texturize_pushpop_element( $text, &$stack, $disabled_elements ) {
...
function autop( $text, $br = true ) {
	...
	$text = replace_in_html_tags( $text, array( "\n" => ' <!-- nl --> ' ) );
	...
	if ( $br ) {
		...
		$text = str_replace( '<PreserveNewline />', "\n", $text );
	}
	...
	if ( str_contains( $text, '<!--nl -->' ) ) {
		$text = str_replace( array( ' <!-- nl --> ', '<!-- nl -->' ), "\n", $text );
	}
 ...
 * @since 4.4.0
 *
 * @param string $shortcode_regex Optional. The result from _get_texturize_shortcode_regex().
 * @return string The regular expression
 */
function _get_texturize_split_regex( $shortcode_regex = '' ) {
	static $html_regex;
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 * @return string The regular expression
 */
function _get_texturize_shortcode_regex( $tagnames ) {
 ...
 */
function _specialchars( $text, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	$text = (string) $text;
	...
	if ( ! $double_encode ) {
		 ...
		 */
		$text = kses_normalize_entities( $text, ( $quote_style & ENT_XML1 ) ? 'xml' : 'html' );
	}
 ...
 *                                Also compatible with old _specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function specialchars_decode( $text, $quote_style = ENT_NOQUOTES ) {
	$text = (string) $text;
	...
	// Match the previous behavior of _specialchars() when the $quote_style is not an accepted value.
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
...
function sanitize_file_name( $filename ) {
	...
	if ( count( $parts ) <= 2 ) {
		/** This filter is documented in includes/functions/formatting.php */
...
function force_balance_tags( $text ) {
	...
	// Sync Site bug fix for comments - in case you REALLY meant to type '< !--'.
	$text = str_replace( '< !--', '<    !--', $text );
	// Sync Site bug fix for LOVE <3 (and other situations with '<' before a number).
	...
	// Sync Site fix for the bug with HTML comments.
...
function _make_clickable_rel_attr( $url ) {
	...
	$nofollow_schemes = array_intersect( allowed_protocols(), array( 'https', 'http' ) );

	// Apply "nofollow" to external links with qualifying URL schemes (mailto:, tel:, etc... shouldn't be followed).
	if ( ! is_internal_link( $url ) && in_array( $scheme, $nofollow_schemes, true ) ) {
		$rel_parts[] = 'nofollow';
	}
 ...
 */
function rel_callback( $matches, $rel ) {
	$text = $matches[1];
	$atts = kses_hair( $matches[1], allowed_protocols() );

	if ( ! empty( $atts['href'] ) && is_internal_link( $atts['href']['value'] ) ) {
...
function rel_nofollow( $text ) {
	...
	$text = preg_replace_callback(
		...
		static function ( $matches ) {
			return rel_callback( $matches, 'nofollow' );
		},
		$text
	);
	return slash( $text );
 ...
 * @return string Converted content.
 */
function rel_ugc( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return rel_callback( $matches, 'nofollow ugc' );
		},
		$text
	);
	return slash( $text );
 ...
 * @return string Converted content.
 */
function targeted_link_rel( $text ) {
	...
	foreach ( $html_parts as &$part ) {
		$part = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'targeted_link_rel_callback', $part );
	}
 ...
 */
function targeted_link_rel_callback( $matches ) {
	$link_html          = $matches[1];
	...
	if ( $is_escaped ) {
		// Replace only the quotes so that they are parsable by kses_hair(), leave the rest as is.
		$link_html = preg_replace( '/\\\\([\'"])/', '$1', $link_html );
	}

	$atts = kses_hair( $link_html, allowed_protocols() );
	 ...
	 */
	$rel = apply_filters( 'targeted_link_rel', 'noopener', $link_html );
 ...
 * @since 5.1.0
 */
function init_targeted_link_rel_filters() {
	$filters = array(
		'title_save_pre',
		'content_save_pre',
		'excerpt_save_pre',
	...
	foreach ( $filters as $filter ) {
		add_filter( $filter, 'targeted_link_rel' );
	}
}
 ...
 * @since 5.1.0
 */
function remove_targeted_link_rel_filters() {
	$filters = array(
		'title_save_pre',
		'content_save_pre',
		'excerpt_save_pre',
	...
	foreach ( $filters as $filter ) {
		remove_filter( $filter, 'targeted_link_rel' );
	}
}
 ...
 */
function iso_descrambler( $subject ) {
	...
	return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_iso_convert', $subject );
}
...
function iso8601_to_datetime( $date_string, $timezone = 'user' ) {
	...
	$timezone = timezone();
	$datetime    = date_create( $date_string, $timezone ); // Timezone is ignored if input has one.
 ...
 * @param Post|object|int $post Optional. Post instance or Post ID/object. Default null.
 * @return string The excerpt.
 */
function trim_excerpt( $text = '', $post = null ) {
	$raw_excerpt = $text;
	...
	if ( '' === trim( $text ) ) {
		...
		/*
		 * Temporarily unhook filter_content_tags() since any tags
		 * within the excerpt are stripped out. Modifying the tags here
		 * is wasteful and can lead to bugs in the image counting logic.
		 */
		$filter_image_removed = remove_filter( 'the_content', 'filter_content_tags', 12 );
		...
		/** This filter is documented in includes/functions/post-template.php */
		 ...
		 * which is generally used for the filter callback in Sync Site core.
		 */
		if ( $filter_image_removed ) {
			add_filter( 'the_content', 'filter_content_tags', 12 );
		}
		...
		$text         = trim_words( $text, $excerpt_length, $excerpt_more );
...
function trim_words( $text, $num_words = 55, $more = null ) {
	...
	if ( str_starts_with( get_word_count_type(), 'characters' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
 ...
 *
 * Usually you should prepare queries using db::prepare().
	...
	$safe_text = (string) preg_replace_callback(
		...
		static function ( $matches ) {
			...
			if ( isset( $matches['non_cdata'] ) ) {
				...
				return _specialchars( $matches['non_cdata'], ENT_XML1 );
			}

			// Return the CDATA Section unchanged, escape HTML entities in the rest.
			return _specialchars( $matches['non_cdata_followed_by_cdata'], ENT_XML1 ) . $matches['cdata'];
		},
		$safe_text
	);
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'new_admin_email':
			$value = $db->strip_invalid_text_for_column( $db->options, 'option_value', $value );
			if ( is_error( $value ) ) {
		...
		case 'blogname':
			...
			if ( $value !== $original_value ) {
				$value = $db->strip_invalid_text_for_column( $db->options, 'option_value', encode_emoji( $original_value ) );
			}
		...
		case 'siteurl':
			...
			if ( is_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The Sync Site address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'LANG':
			...
			if ( ! is_multisite() && defined( 'LANG' ) && '' !== LANG && 'en_US' !== LANG ) {
				$allowed[] = LANG;
			}
 ...
 */
function sync_parse_str( $input_string, &$result ) {
	 ...
	 */
	$result = apply_filters( 'sync_parse_str', $result );
}
 ...
 * @return string Converted text.
 */
function pre_kses_less_than( $content ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'pre_kses_less_than_callback', $content );
}
 ...
 */
function pre_kses_block_attributes( $content, $allowed_html, $allowed_protocols ) {
	/*
	 * `filter_block_content` is expected to call `kses`. Temporarily remove
	 * the filter to avoid recursion.
	 */
	remove_filter( 'pre_kses', 'pre_kses_block_attributes', 10 );
...
/**
 * Sync Site' implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
 ...
 */
function sync_sprintf( $pattern, ...$args ) {
	$len       = strlen( $pattern );
	$start     = 0;
	$result    = '';
	$arg_index = 0;
	...
	while ( $len > $start ) {
		...
		if ( '%' === $pattern[ $start ] ) {
			...
			/**
			 * Filters a fragment from the pattern passed to sync_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'sync_sprintf', $fragment, $arg );
 ...
 */
function sync_sprintf_l( $pattern, $args ) {
	// Not a match.
	...
	/**
	 * Filters the translated delimiters used by sync_sprintf_l().
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], allowed_protocols(), true ) ?
			$m[3] :
...
function strip_all_tags( $text, $remove_breaks = false ) {
	...
	if ( ! is_scalar( $text ) ) {
		 ...
		 * sync_trigger_error() is used to trigger an E_USER_WARNING,
 ...
 * @return string
 */
function sync_basename( $path, $suffix = '' ) {
...
function spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
...
function enqueue_emoji_styles() {
	...
	$action = is_admin() ? 'admin_print_styles' : 'print_styles';
	...
	$handle       = 'emoji-styles';
	register_style( $handle, false );
	add_inline_style( $handle, $emoji_styles );
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			/** This filter is documented in includes/classes/class-scripts.php */
			'syncemoji' => apply_filters( 'script_loader_src', includes_url( "js/emoji.js?$version" ), 'syncemoji' ),
			/** This filter is documented in includes/classes/class-scripts.php */
	...
	print_inline_script_tag(
		sprintf( 'window._emojiSettings = %s;', sync_json_encode( $settings ) ) . "\n" .
			file_get_contents( INC . 'assets/js/emoji-loader' . scripts_get_suffix() . '.js' )
	);
}
 ...
 */
function staticize_emoji_for_email( $mail ) {
	 ...
	 * in sync_mail() - get the `Content-Type` from the headers, if there is one,
	 * then pass it through the {@see 'mail_content_type'} filter, in case
	...
	/** This filter is documented in includes/functions/pluggable.php */
	$content_type = apply_filters( 'mail_content_type', $content_type );

	if ( 'text/html' === $content_type ) {
		$mail['message'] = staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that Sync Site recognises.
 */
function _emoji_list( $type = 'entities' ) {
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to includes/functions/formatting.php from admin/includes/misc.php and added $length param.
 *
