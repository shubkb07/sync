...
require CLASSES . 'class-metadata-lazyloader.php';
...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	...
	$meta_key   = unslash( $meta_key );
	$meta_value = unslash( $meta_value );
	...
	cache_delete( $object_id, $meta_type . '_meta' );
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	cache_delete_multiple( $data, $meta_type . '_meta' );
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in includes/functions/meta.php */
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		/** This action is documented in includes/functions/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	$cache          = array();
	$cache_values   = cache_get_multiple( $object_ids, $cache_key );
	...
	cache_add_multiple( $data, $cache_key );
 ...
 * @since 4.5.0
 *
 * @return Metadata_Lazyloader Metadata lazyloader queue.
 */
function metadata_lazyloader() {
	static $metadata_lazyloader;

	if ( null === $metadata_lazyloader ) {
		$metadata_lazyloader = new Metadata_Lazyloader();
	}

	return $metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new Meta_Query( $meta_query );
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		...
		$meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] = $args;
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $meta_keys ) || ! isset( $meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $meta_keys[ $object_type ] ) ) {
		unset( $meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type, $object_subtype = '' ) {
	...
	if ( ! is_array( $meta_keys ) || ! isset( $meta_keys[ $object_type ] ) || ! isset( $meta_keys[ $object_type ][ $object_subtype ] ) ) {
		return array();
	}

	return $meta_keys[ $object_type ][ $object_subtype ];
}
 ...
 */
function _register_meta_args_allowed_list( $args, $default_args ) {
