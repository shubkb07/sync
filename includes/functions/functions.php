 ...
 * be passed to `sync_date()` for translation.
 *
 * @since 0.71
 *
...
function mysql2date( $format, $date, $translate = true ) {
	...
	if ( $translate ) {
		return sync_date( $format, $datetime->getTimestamp(), $timezone );
	}
...
function current_time( $type, $gmt = 0 ) {
	...
	$timezone = $gmt ? new DateTimeZone( 'UTC' ) : timezone();
 ...
 *
 * Note that due to the way Sync Site typically generates a sum of timestamp and offset
 * with `strtotime()`, it implies offset added at a _current_ time, not at the time
 * the timestamp represents. Storing such timestamps or calculating them differently
 * will lead to invalid output.
 *
 * @since 0.71
 * @since 5.3.0 Converted into a wrapper for sync_date().
 *
...
function date_i18n( $format, $timestamp_with_offset = false, $gmt = false ) {
	...
	if ( 'U' === $format ) {
		...
		$date = sync_date( $format, null, new DateTimeZone( 'UTC' ) );
	} elseif ( false === $timestamp_with_offset ) { // Current time in site's timezone.
		$date = sync_date( $format );
	} else {
		/*
		...
		$date       = sync_date( $format, $datetime->getTimestamp(), $timezone );
	}
 ...
 */
function sync_date( $format, $timestamp = null, $timezone = null ) {
	global $locale;
	...
	if ( empty( $locale->month ) || empty( $locale->weekday ) ) {
		...
		$month         = $locale->get_month( $datetime->format( 'm' ) );
		$weekday       = $locale->get_weekday( $datetime->format( 'w' ) );

		for ( $i = 0; $i < $format_length; $i++ ) {
			switch ( $format[ $i ] ) {
				case 'D':
					$new_format .= addcslashes( $locale->get_weekday_abbrev( $weekday ), '\\A..Za..z' );
					break;
				case 'F':
				...
				case 'l':
					...
					break;
				case 'M':
					$new_format .= addcslashes( $locale->get_month_abbrev( $month ), '\\A..Za..z' );
					break;
				case 'a':
					$new_format .= addcslashes( $locale->get_meridiem( $datetime->format( 'a' ) ), '\\A..Za..z' );
					break;
				case 'A':
					$new_format .= addcslashes( $locale->get_meridiem( $datetime->format( 'A' ) ), '\\A..Za..z' );
					break;
		...
		$date = maybe_decline_date( $date, $format );
	}
	 ...
	 */
	$date = apply_filters( 'sync_date', $date, $format, $timestamp, $timezone );
 ...
 */
function maybe_decline_date( $date, $format = '' ) {
	global $locale;
	...
	if ( 'on' === _x( 'off', 'decline months names: on or off' ) ) {
		...
		$months          = $locale->month;
		$months_genitive = $locale->month_genitive;
...
function number_format_i18n( $number, $decimals = 0 ) {
	...
	if ( isset( $locale ) ) {
		$formatted = number_format( $number, absint( $decimals ), $locale->number_format['decimal_point'], $locale->number_format['thousands_sep'] );
	} else {
 ...
 *              updated to accept a post ID or a Post object.
...
function do_enclose( $content, $post ) {
	...
	require_once INC . 'classes/class-IXR.php';
	...
	$post_links_temp = extract_urls( $content );

	foreach ( $pung as $link_test ) {
		// Link is no longer in post.
		if ( ! in_array( $link_test, $post_links_temp, true ) ) {
			$mids = $db->get_col( $db->prepare( "SELECT meta_id FROM $db->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE %s", $post->ID, $db->esc_like( $link_test ) . '%' ) );
			foreach ( $mids as $mid ) {
	...
	foreach ( (array) $post_links as $url ) {
		...
		if ( '' !== $url && ! $db->get_var( $db->prepare( "SELECT post_id FROM $db->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE %s", $post->ID, $db->esc_like( $url ) . '%' ) ) ) {
			...
			if ( $headers ) {
				...
				if ( false !== $url_parts && ! empty( $url_parts['path'] ) ) {
					...
					if ( ! empty( $extension ) ) {
						foreach ( get_mime_types() as $exts => $mime ) {
 ...
 * @return \Sync\Requests\Utility\CaseInsensitiveDictionary|false Headers on success, false on failure.
 */
function get_http_headers( $url, $deprecated = false ) {
	if ( ! empty( $deprecated ) ) {
		_deprecated_argument( __FUNCTION__, '2.7.0' );
	}

	$response = safe_remote_head( $url );

	if ( is_error( $response ) ) {
		return false;
	}

	return remote_retrieve_headers( $response );
}
...
function remote_fopen( $uri ) {
	...
	$response = safe_remote_get( $uri, $options );

	if ( is_error( $response ) ) {
		return false;
	}

	return remote_retrieve_body( $response );
}

/**
 * Sets up the Sync Site query.
 *
 * @since 2.0.0
 *
 * @global Sync Site       $sync           Current Sync Site environment instance.
 * @global Query $query     Sync Site Query object.
 * @global Query $the_query Copy of the Sync Site Query object.
 *
 * @param string|array $query_vars Default Query arguments.
 */
function sync( $query_vars = '' ) {
	global $sync, $query, $the_query;

	$sync->main( $query_vars );
...
/**
 * Retrieves the number of database queries during the Sync Site execution.
 *
 * @since 2.0.0
 *
 * @global db $db Sync Site database abstraction object.
...
function do_feed() {
	...
	if ( ! has_action( "do_feed_{$feed}" ) ) {
		sync_die( __( '<strong>Error:</strong> This is not a valid feed template.' ), '', array( 'response' => 404 ) );
	}
	 ...
	 */
	do_action( "do_feed_{$feed}", $query->is_comment_feed, $feed );
}
...
function do_feed_rdf() {
	load_template( INC . '/feed-rdf.php' );
}
...
function do_feed_rss() {
	load_template( INC . '/feed-rss.php' );
}
...
function do_feed_rss2( $for_comments ) {
	if ( $for_comments ) {
		load_template( INC . '/feed-rss2-comments.php' );
	} else {
		load_template( INC . '/feed-rss2.php' );
	}
}
...
function do_feed_atom( $for_comments ) {
	if ( $for_comments ) {
		load_template( INC . '/feed-atom-comments.php' );
	} else {
		load_template( INC . '/feed-atom.php' );
	}
}
 ...
 *              discouraged in favor of robots meta HTML tag via robots_no_robots()
 *              filter callback.
 */
function do_robots() {
...
function do_favicon() {
	...
	redirect( get_site_icon_url( 32, includes_url( 'images/w-logo-blue-white-bg.png' ) ) );
	exit;
}

/**
 * Determines whether Sync Site is already installed.
 *
 * The cache will be checked first. If you have a cache plugin, which saves
 * the cache values, then this will work. If you use the default Sync Site
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether Sync Site is installed.
 *
 * For more information on this and similar theme functions, check out
 * Conditional Tags} article in the Theme Developer Handbook.
 *
...
function is_blog_installed() {
	 ...
	 * cached, oh well.
	 */
	if ( cache_get( 'is_blog_installed' ) ) {
		return true;
	}
	...
	if ( ! isset( $alloptions['siteurl'] ) ) {
		$installed = $db->get_var( "SELECT option_value FROM $db->options WHERE option_name = 'siteurl'" );
	} else {
	...
	cache_set( 'is_blog_installed', $installed );
	...
	/*
	 * Loop over the Sync Site tables. If none exist, then scratch installation is allowed.
	...
	foreach ( $tables as $table ) {
		...
		$described_table = $db->get_results( "DESCRIBE $table;" );
		if (
			( ! $described_table && empty( $db->last_error ) ) ||
	...
	cache_set( 'is_blog_installed', false );
 ...
 * @param string     $name      Optional. Nonce name. Default '_nonce'.
 * @return string Escaped URL with nonce action added.
 */
function nonce_url( $actionurl, $action = -1, $name = '_nonce' ) {
	$actionurl = str_replace( '&amp;', '&', $actionurl );
	return esc_html( add_query_arg( $name, create_nonce( $action ), $actionurl ) );
}
 ...
 */
function nonce_field( $action = -1, $name = '_nonce', $referer = true, $display = true ) {
	$name        = esc_attr( $name );
	$nonce_field = '<input type="hidden" id="' . $name . '" name="' . $name . '" value="' . create_nonce( $action ) . '" />';

	if ( $referer ) {
		$nonce_field .= referer_field( false );
	}
 ...
 */
function referer_field( $display = true ) {
	$request_url   = remove_query_arg( '_Http_referer' );
	$referer_field = '<input type="hidden" name="_Http_referer" value="' . esc_url( $request_url ) . '" />';
 ...
 *
 * The input name is '_original_http_referer' and will be either the same
 * value of referer_field(), if that was posted already or it will be the
 ...
 */
function original_referer_field( $display = true, $jump_back_to = 'current' ) {
	$ref = get_original_referer();

	if ( ! $ref ) {
		$ref = ( 'previous' === $jump_back_to ) ? get_referer() : unslash( $_SERVER['REQUEST_URI'] );
	}

	$orig_referer_field = '<input type="hidden" name="_original_http_referer" value="' . esc_attr( $ref ) . '" />';
...
/**
 * Retrieves referer from '_Http_referer' or HTTP referer.
 *
...
function get_referer() {
	// Return early if called before validate_redirect() is defined.
	if ( ! function_exists( 'validate_redirect' ) ) {
		return false;
	}

	$ref = get_raw_referer();

	if ( $ref && unslash( $_SERVER['REQUEST_URI'] ) !== $ref
		&& home_url() . unslash( $_SERVER['REQUEST_URI'] ) !== $ref
	) {
		return validate_redirect( $ref, false );
	}
...
/**
 * Retrieves unvalidated referer from the '_Http_referer' URL query variable or the HTTP referer.
 *
 * If the value of the '_Http_referer' URL query variable is not a string then it will be ignored.
 *
 * Do not use for redirects, use get_referer() instead.
 *
 * @since 4.5.0
 *
 * @return string|false Referer URL on success, false on failure.
 */
function get_raw_referer() {
	if ( ! empty( $_REQUEST['_Http_referer'] ) && is_string( $_REQUEST['_Http_referer'] ) ) {
		return unslash( $_REQUEST['_Http_referer'] );
	} elseif ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		return unslash( $_SERVER['HTTP_REFERER'] );
	}
 ...
 */
function get_original_referer() {
	// Return early if called before validate_redirect() is defined.
	if ( ! function_exists( 'validate_redirect' ) ) {
		return false;
	}

	if ( ! empty( $_REQUEST['_original_http_referer'] ) ) {
		return validate_redirect( unslash( $_REQUEST['_original_http_referer'] ), false );
	}
...
function path_is_absolute( $path ) {
	 ...
	 */
	if ( is_stream( $path ) && ( is_dir( $path ) || is_file( $path ) ) ) {
		return true;
	}
 ...
 * followed by your PHP temporary upload directory, followed by CONTENT_DIR,
 * before finally defaulting to /tmp/
 *
 * In the event that this function does not find a writable location,
 * It may be overridden by the TEMP_DIR constant in your sync-config.php file.
 *
 * @since 2.5.0
 *
...
function get_temp_dir() {
	...
	if ( function_exists( 'sys_get_temp_dir' ) ) {
		...
		if ( @is_dir( $temp ) && sync_is_writable( $temp ) ) {
			return trailingslashit( $temp );
		}
	}
 ...
 */
function sync_is_writable( $path ) {
 ...
 *
 * Same as upload_dir() but "light weight" as it doesn't attempt to create the uploads directory.
 ...
 * @see upload_dir()
 *
 * @return array See upload_dir() for description.
 */
function get_upload_dir() {
	return upload_dir( null, false );
}
 ...
 * 'CONTENT_DIR/uploads'. If the 'UPLOADS' constant is defined, then it will
 * override the 'upload_path' option and 'CONTENT_DIR/uploads' path.
 *
 * The upload URL path is set either by the 'upload_url_path' option or by using
 * the 'CONTENT_URL' constant and appending '/uploads' to the path.
 *
 ...
 * }
 */
function upload_dir( $time = null, $create_dir = true, $refresh_cache = false ) {
	static $cache = array(), $tested_paths = array();

	$key = sprintf( '%d-%s', get_current_blog_id(), (string) $time );

	if ( $refresh_cache || empty( $cache[ $key ] ) ) {
		$cache[ $key ] = _upload_dir( $time );
	}
		...
		if ( array_key_exists( $path, $tested_paths ) ) {
			...
			if ( ! mkdir_p( $path ) ) {
				if ( str_starts_with( $uploads['basedir'], ABSPATH ) ) {
					...
					$error_path = sync_basename( $uploads['basedir'] ) . $uploads['subdir'];
				}
...
/**
 * A non-filtered, non-cached version of upload_dir() that doesn't check the path.
 *
 * @since 4.5.0
 * @access private
 *
 * @param string|null $time Optional. Time formatted in 'yyyy/mm'. Default null.
 * @return array See upload_dir()
 */
function _upload_dir( $time = null ) {
	$siteurl     = get_option( 'siteurl' );
	$upload_path = trim( get_option( 'upload_path' ) );

	if ( empty( $upload_path ) || 'public/uploads' === $upload_path ) {
		$dir = CONTENT_DIR . '/uploads';
	...
	if ( ! $url ) {
		if ( empty( $upload_path ) || ( 'public/uploads' === $upload_path ) || ( $upload_path === $dir ) ) {
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had public/uploads for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as public/uploads is used
			 *    there, and
			 ...
			 * rewriting in multisite, the resulting URL is /files.
			 */
...
function unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	// Initialize vars used in the unique_filename filter.
	$number        = '';
	$alt_filenames = array();
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		...
		/*
		 * Get the mime type. Uploaded files were already checked with check_filetype_and_ext()
		 * in _handle_upload(). Using check_filetype() would be sufficient here.
		 */
		 ...
		 * for better compatibility with different filesystems.
		 */
		if ( $ext && $lc_ext !== $ext ) {
		...
		if ( $name && $ext && @is_dir( $dir ) && str_contains( $dir, $upload_dir['basedir'] ) ) {
			 ...
			 */
			$files = apply_filters( 'pre_unique_filename_file_list', null, $dir, $filename );
			...
			if ( ! empty( $files ) ) {
				...
				while ( $i <= $count && _check_existing_file_names( $filename, $files ) ) {
					$new_number = (int) $number + 1;
		 ...
		 */
		if ( $is_image ) {
			/** This filter is documented in includes/classes/class-image-editor.php */
			...
			foreach ( $alt_types as $alt_type ) {
				$alt_ext = get_default_extension_for_mime_type( $alt_type );
			...
			if ( ! empty( $alt_filenames ) ) {
				...
				while ( $i <= $count && _check_alternate_file_names( $alt_filenames, $_dir, $files ) ) {
					$new_number = (int) $number + 1;
	 ...
	 */
	return apply_filters( 'unique_filename', $filename, $ext, $dir, $unique_filename_callback, $alt_filenames, $number );
}
 ...
 */
function _check_alternate_file_names( $filenames, $dir, $files ) {
	foreach ( $filenames as $filename ) {
		if ( file_exists( $dir . $filename ) ) {
			return true;
		}

		if ( ! empty( $files ) && _check_existing_file_names( $filename, $files ) ) {
			return true;
		}
	}
 ...
 * @return bool True if the tested file name could match an existing file, false otherwise.
 */
function _check_existing_file_names( $filename, $files ) {
	$fname = pathinfo( $filename, PATHINFO_FILENAME );
	$ext   = pathinfo( $filename, PATHINFO_EXTENSION );
 ...
 * }
 */
function upload_bits( $name, $deprecated, $bits, $time = null ) {
	if ( ! empty( $deprecated ) ) {
	...
	$filename = unique_filename( $upload['path'], $name );

	$new_file = $upload['path'] . "/$filename";
	if ( ! mkdir_p( dirname( $new_file ) ) ) {
		if ( str_starts_with( $upload['basedir'], ABSPATH ) ) {
			$error_path = str_replace( ABSPATH, '', $upload['basedir'] ) . $upload['subdir'];
		} else {
			$error_path = sync_basename( $upload['basedir'] ) . $upload['subdir'];
		}
 ...
 * @return string|false
 */
function get_default_extension_for_mime_type( $mime_type ) {
	$extensions = explode( '|', array_search( $mime_type, get_mime_types(), true ) );
 ...
 *
 * Currently this function only supports renaming images validated via get_image_mime().
 *
 * @since 3.0.0
 *
...
function check_filetype_and_ext( $file, $filename, $mimes = null ) {
	...
	if ( $type && str_starts_with( $type, 'image/' ) ) {
		...
		if ( $real_mime && $real_mime !== $type ) {
			...
			if ( ! empty( $mime_to_ext[ $real_mime ] ) ) {
				...
				$filetype = check_filetype( $new_filename, $mimes );
				$ext         = $filetype['ext'];
				$type        = $filetype['type'];
			} else {
	 ...
	 *
	 * @param array         $check_filetype_and_ext {
	 *     Values for the extension, mime type, and corrected filename.
	 *
	 *     @type string|false $ext             File extension, or false if the file doesn't match a mime type.
	 *     @type string|false $type            File mime type, or false if the file doesn't match a mime type.
	 ...
	 */
	return apply_filters( 'check_filetype_and_ext', compact( 'ext', 'type', 'proper_filename' ), $file, $filename, $mimes, $real_mime );
}
	...
	try {
		if ( is_callable( 'exif_imagetype' ) ) {
			...
			if ( defined( 'DEBUG' ) && DEBUG
				&& ! defined( 'RUN_CORE_TESTS' )
			) {
				// Not using getimagesize() here to avoid an infinite loop.
...
function get_mime_types() {
	 ...
	 * @since 3.5.0
	 *
	 * @param string[] $get_mime_types Mime types keyed by the file extension regex
 ...
 */
function sync_filesize( $path ) {
	/**
	 * Filters the result of sync_filesize before the PHP function is run.
	 *
	 * @since 6.0.0
	 *
	 ...
	 * @param string   $path Path to the file.
	 */
	$size = apply_filters( 'pre_sync_filesize', null, $path );
	 ...
	 * @param string $path Path to the file.
	 */
	return (int) apply_filters( 'sync_filesize', $size, $path );
}
...
function nonce_ays( $action ) {
	...
	if ( 'log-out' === $action ) {
		...
		$html .= sprintf(
			...
			logout_url( $redirect_to )
		);
	} else {
		$html = __( 'The link you followed has expired.' );

		if ( get_referer() ) {
			$Http_referer = remove_query_arg( 'updated', get_referer() );
			$Http_referer = validate_redirect( sanitize_url( $Http_referer ) );

			$html .= '</p><p>';
			$html .= sprintf(
				'<a href="%s">%s</a>',
				esc_url( $Http_referer ),
				__( 'Please try again.' )
			);
		}
	}

	sync_die( $html, $title, $response_code );
}

/**
 * Kills Sync Site execution and displays HTML page with an error message.
 *
 ...
 * @global Query $query Sync Site Query object.
 *
 * @param string|Sync_Error  $message Optional. Error message. If this is a Sync_Error object,
 *                                  and not an Ajax or XML-RPC request, the error's messages are used.
 *                                  Default empty string.
 * @param string|int       $title   Optional. Error title. If `$message` is a `Sync_Error` object,
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when Sync Site is still
 *                                  loading and the site's locale is not set up yet. Accepts 'rtl' and 'ltr'.
 *                                  Default is the value of is_rtl().
 *     @type string $charset        Character set of the HTML output. Default 'utf-8'.
 *     @type string $code           Error code to use. Default is 'sync_die', or the main error code if $message
 *                                  is a Sync_Error.
 *     @type bool   $exit           Whether to exit the process after completion. Default true.
 * }
 */
function sync_die( $message = '', $title = '', $args = array() ) {
	global $query;
	...
	if ( doing_ajax() ) {
		/**
		 * Filters the callback for killing Sync Site execution for Ajax requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'sync_die_ajax_handler', '_ajax_sync_die_handler' );
	} elseif ( is_json_request() ) {
		/**
		 * Filters the callback for killing Sync Site execution for JSON requests.
		 *
		 * @since 5.1.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'sync_die_json_handler', '_json_sync_die_handler' );
	} elseif ( is_serving_rest_request() && is_jsonp_request() ) {
		/**
		 * Filters the callback for killing Sync Site execution for JSONP REST requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'sync_die_jsonp_handler', '_jsonp_sync_die_handler' );
	} elseif ( defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST ) {
		/**
		 * Filters the callback for killing Sync Site execution for XML-RPC requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'sync_die_xmlrpc_handler', '_xmlrpc_sync_die_handler' );
	} elseif ( is_xml_request()
		|| isset( $query ) &&
			( function_exists( 'is_feed' ) && is_feed()
			|| function_exists( 'is_comment_feed' ) && is_comment_feed()
			|| function_exists( 'is_trackback' ) && is_trackback() ) ) {
		/**
		 * Filters the callback for killing Sync Site execution for XML requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'sync_die_xml_handler', '_xml_sync_die_handler' );
	} else {
		/**
		 * Filters the callback for killing Sync Site execution for all non-Ajax, non-JSON, non-XML requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'sync_die_handler', '_default_sync_die_handler' );
	}
 ...
 * Kills Sync Site execution and displays HTML page with an error message.
 *
 * This is the default handler for sync_die(). If you want a custom one,
 * you can override this using the {@see 'sync_die_handler'} filter in sync_die().
 *
 * @since 3.0.0
 * @access private
 *
 * @param string|Sync_Error $message Error message or Sync_Error object.
 * @param string          $title   Optional. Error title. Default empty string.
 * @param string|array    $args    Optional. Arguments to control behavior. Default empty array.
 */
function _default_sync_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _sync_die_process_input( $message, $title, $args );

	if ( is_string( $message ) ) {
		if ( ! empty( $parsed_args['additional_errors'] ) ) {
			$message = array_merge(
				array( $message ),
				list_pluck( $parsed_args['additional_errors'], 'message' )
			);
			$message = "<ul>\n\t\t<li>" . implode( "</li>\n\t\t<li>", $message ) . "</li>\n\t</ul>";
		}
...
<head>
	...
	<meta name="viewport" content="width=device-width">
		<?php
		if ( function_exists( 'robots' ) && function_exists( 'robots_no_robots' ) && function_exists( 'add_filter' ) ) {
			add_filter( 'robots', 'robots_no_robots' );
			// Prevent warnings because of $query not existing.
			remove_filter( 'robots', 'robots_noindex_embeds' );
			remove_filter( 'robots', 'robots_noindex_search' );
			robots();
		}
		?>
...
/**
 * Kills Sync Site execution and displays Ajax response with an error message.
 *
 * This is the handler for sync_die() when processing Ajax requests.
 *
 * @since 3.4.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _ajax_sync_die_handler( $message, $title = '', $args = array() ) {
...
/**
 * Kills Sync Site execution and displays JSON response with an error message.
 *
 * This is the handler for sync_die() when processing JSON requests.
 *
 * @since 5.1.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _json_sync_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _sync_die_process_input( $message, $title, $args );
...
/**
 * Kills Sync Site execution and displays JSONP response with an error message.
 *
 * This is the handler for sync_die() when processing JSONP requests.
 *
 * @since 5.2.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _jsonp_sync_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _sync_die_process_input( $message, $title, $args );
...
/**
 * Kills Sync Site execution and displays XML response with an error message.
 *
 * This is the handler for sync_die() when processing XMLRPC requests.
 *
 * @since 3.2.0
 * @access private
 *
 * @global xmlrpc_server $xmlrpc_server
 *
 * @param string       $message Error message.
 * @param string       $title   Optional. Error title. Default empty string.
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _xmlrpc_sync_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( $xmlrpc_server ) {
		...
		$xmlrpc_server->output( $error->getXml() );
	}
	if ( $parsed_args['exit'] ) {
		die();
	}
 ...
 * Kills Sync Site execution and displays XML response with an error message.
 *
 * This is the handler for sync_die() when processing XML requests.
 *
 * @since 5.2.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _xml_sync_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _sync_die_process_input( $message, $title, $args );
...
/**
 * Kills Sync Site execution and displays an error message.
 *
 * This is the handler for sync_die() when processing APP requests.
 *
 * @since 3.4.0
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _scalar_sync_die_handler( $message = '', $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _sync_die_process_input( $message, $title, $args );
...
/**
 * Processes arguments passed to sync_die() consistently for its handlers.
 *
 * @since 5.1.0
 * @access private
 *
 ...
 * }
 */
function _sync_die_process_input( $message, $title = '', $args = array() ) {
	$defaults = array(
		'response'          => 0,
		'code'              => '',
		'exit'              => true,
	...
	if ( function_exists( 'is_error' ) && is_error( $message ) ) {
		if ( ! empty( $message->errors ) ) {
			...
			if ( DEBUG_DISPLAY && is_array( $errors[0]['data'] ) && ! empty( $errors[0]['data']['error'] ) ) {
	...
	if ( empty( $title ) ) {
		$title = $have_gettext ? __( 'Sync &rsaquo; Error' ) : 'Sync &rsaquo; Error';
	}
 ...
 */
function sync_json_encode( $value, $flags = 0, $depth = 512 ) {
	...
	try {
		$value = _json_sanity_check( $value, $depth );
	} catch ( Exception $e ) {
		return false;
	}
 ...
 */
function _json_sanity_check( $value, $depth ) {
	if ( $depth < 0 ) {
	...
	if ( is_array( $value ) ) {
		...
		foreach ( $value as $id => $el ) {
			...
			if ( is_string( $id ) ) {
				$clean_id = _json_convert_string( $id );
			} else {
				$clean_id = $id;
			}
			...
			if ( is_array( $el ) || is_object( $el ) ) {
				$output[ $clean_id ] = _json_sanity_check( $el, $depth - 1 );
			} elseif ( is_string( $el ) ) {
				$output[ $clean_id ] = _json_convert_string( $el );
			} else {
				$output[ $clean_id ] = $el;
			}
		}
		...
		return _json_convert_string( $value );
	} else {
		return $value;
	}
...
function _json_convert_string( $input_string ) {
	...
	if ( $use_mb ) {
		...
		return check_invalid_utf8( $input_string, true );
	}
}
 ...
 */
function _json_prepare_data( $value ) {
 ...
 * @param int   $flags       Optional. Options to be passed to json_encode(). Default 0.
 */
function send_json( $response, $status_code = null, $flags = 0 ) {
	if ( is_serving_rest_request() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: REST_Response, 2: Sync_Error */
	...
	echo sync_json_encode( $response, $flags );
 ...
 * @param int   $flags       Optional. Options to be passed to json_encode(). Default 0.
 */
function send_json_success( $value = null, $status_code = null, $flags = 0 ) {
	...
	send_json( $response, $status_code, $flags );
}

/**
 * Sends a JSON response back to an Ajax request, indicating failure.
 *
 * If the `$value` parameter is a Sync_Error object, the errors
 * within the object are processed and output as an array of error
 * codes and corresponding messages. All other types are output
 * without further processing.
 *
 * @since 3.5.0
 * @since 4.1.0 The `$value` parameter is now processed if a Sync_Error object is passed in.
 * @since 4.7.0 The `$status_code` parameter was added.
 * @since 5.6.0 The `$flags` parameter was added.
 *
 * @param mixed $value       Optional. Data to encode as JSON, then print and die. Default null.
 ...
 * @param int   $flags       Optional. Options to be passed to json_encode(). Default 0.
 */
function send_json_error( $value = null, $status_code = null, $flags = 0 ) {
 ...
 */
function check_jsonp_callback( $callback ) {
 ...
 */
function json_file_decode( $filename, $options = array() ) {
	$result   = null;
	$filename = normalize_path( realpath( $filename ) );
	...
	$options      = parse_args( $options, array( 'associative' => false ) );
...
/**
 * Retrieves the Sync Site home page URL.
 *
 * If the constant named 'HOME' exists, then it will be used and returned
...
/**
 * Retrieves the Sync Site site URL.
 *
 * If the constant named 'SITEURL' is defined, then the value in that
 ...
 * @see SITEURL
 *
 * @param string $url URL to set the Sync Site site location.
 * @return string The Sync Site site URL.
 */
function _config_SITEURL( $url = '' ) {
	if ( defined( 'SITEURL' ) ) {
 ...
 *
 * Will only set the direction to 'rtl', if the Sync Site locale has
...
/**
 * Determines whether Sync Site is currently serving a REST API request.
 *
 ...
 * REST response. See {@see is_rest_endpoint()} for that purpose.
 *
 ...
 * @since 6.5.0
 *
 * @return bool True if it's a Sync Site REST API request, false otherwise.
 */
function is_serving_rest_request() {
 ...
 * @global array $smiliessearch
 */
function smilies_init() {
	global $smiliestrans, $smiliessearch;
	 ...
	 * @since 4.7.0
	 *
	 * @param string[] $smiliestrans List of the smilies' hexadecimal representations, keyed by their smily code.
	 */
	$smiliestrans = apply_filters( 'smilies', $smiliestrans );

	if ( count( $smiliestrans ) === 0 ) {
		return;
	}
 ...
 *
 * This function is used throughout Sync Site to allow for both string or array
 ...
 */
function parse_list( $input_list ) {
 ...
 *
 * @since 3.0.0
 * @since 5.1.0 Refactored to use parse_list().
 *
 * @param array|string $input_list List of IDs.
 * @return int[] Sanitized array of IDs.
 */
function parse_id_list( $input_list ) {
	$input_list = parse_list( $input_list );
 ...
 */
function parse_slug_list( $input_list ) {
	$input_list = parse_list( $input_list );
 ...
 */
function array_slice_assoc( $input_array, $keys ) {
	$slice = array();
 ...
 */
function recursive_ksort( &$input_array ) {
 ...
 *         ),
 *     );
 *     _array_get( $input_array, array( 'a', 'b', 'c' ) );
 *
 * @internal
 *
 * @since 5.6.0
 ...
 */
function _array_get( $input_array, $path, $default_value = null ) {
 ...
 *
 *     $input_array = array();
 *     _array_set( $input_array, array( 'a', 'b', 'c', 1 ) );
 *
 *     $input_array becomes:
 *     array(
 *         'a' => array(
 ...
 */
function _array_set( &$input_array, $path, $value = null ) {
 ...
 */
function _to_kebab_case( $input_string ) {
 ...
 * a particular field from all matching objects, whereas list_filter()
 * only does the filtering.
 *
 * @since 3.0.0
 * @since 4.7.0 Uses `List_Util` class.
 *
 ...
 */
function filter_object_list( $input_list, $args = array(), $operator = 'and', $field = false ) {
	if ( ! is_array( $input_list ) ) {
		return array();
	}

	$util = new List_Util( $input_list );
 ...
 * use filter_object_list() instead.
 *
 * @since 3.1.0
 * @since 4.7.0 Uses `List_Util` class.
 * @since 5.9.0 Converted into a wrapper for `filter_object_list()`.
 *
 * @param array  $input_list An array of objects to filter.
 * @param array  $args       Optional. An array of key => value arguments to match
 *                           against each object. Default empty array.
 ...
 */
function list_filter( $input_list, $args = array(), $operator = 'AND' ) {
	return filter_object_list( $input_list, $args, $operator );
}
 ...
 */
function list_pluck( $input_list, $field, $index_key = null ) {
	if ( ! is_array( $input_list ) ) {
		return array();
	}
 ...
 */
function list_sort( $input_list, $orderby = array(), $order = 'ASC', $preserve_keys = false ) {
	if ( ! is_array( $input_list ) ) {
		return array();
	}
...
function maybe_load_widgets() {
	 ...
	 * @since 2.8.0
	 *
	 * @param bool $maybe_load_widgets Whether to load the Widgets library.
	 *                                    Default true.
	 */
	...
	require_once INC . '/default-widgets.php';

	add_action( '_admin_menu', 'widgets_add_menu' );
}
...
/**
 * Loads custom DB error or display Sync Site DB error.
 *
 * If a file exists in the public directory named db-error.php, then it will
 * be loaded instead of displaying the Sync Site DB error. If it is not found,
 * then the Sync Site DB error will be displayed instead.
 *
 * The Sync Site DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to Sync Site 2.3.2, but originally was added
 * in Sync Site 2.5.0.
 *
 * @since 2.3.2
 *
...
function dead_db() {
	...
	if ( installing() || defined( 'ADMIN' ) ) {
		sync_die( $db->error );
	}

	// Otherwise, be terse.
	sync_die( '<h1>' . __( 'Error establishing a database connection' ) . '</h1>', __( 'Database Error' ) );
}
 ...
 * @param string $version       The version of Sync Site that deprecated the function.
...
function _deprecated_function( $function_name, $version, $replacement = '' ) {
	 ...
	 */
	if ( DEBUG && apply_filters( 'deprecated_function_sync_trigger_error', true ) ) {
...
function _deprecated_constructor( $class_name, $version, $parent_class = '' ) {
	 ...
	 * Filters whether to trigger an error for deprecated functions.
	 *
	 * `DEBUG` must be true in addition to the filter evaluating to true.
	 *
	 * @since 4.3.0
	 *
	 * @param bool $trigger Whether to trigger the error for deprecated functions. Default true.
	 */
	if ( DEBUG && apply_filters( 'deprecated_constructor_sync_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $parent_class ) {
				$message = sprintf(
 ...
 * @param string $version     The version of Sync Site that deprecated the class.
...
function _deprecated_class( $class_name, $version, $replacement = '' ) {
	 ...
	 */
	if ( DEBUG && apply_filters( 'deprecated_class_sync_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $replacement ) {
				$message = sprintf(
 ...
 * @param string $version     The version of Sync Site that deprecated the file.
...
function _deprecated_file( $file, $version, $replacement = '', $message = '' ) {
	 ...
	 */
	if ( DEBUG && apply_filters( 'deprecated_file_sync_trigger_error', true ) ) {
 ...
 *
 * @param string $function_name The function that was called.
 * @param string $version       The version of Sync Site that deprecated the argument used.
 * @param string $message       Optional. A message regarding the change. Default empty string.
 */
function _deprecated_argument( $function_name, $version, $message = '' ) {
	 ...
	 */
	if ( DEBUG && apply_filters( 'deprecated_argument_sync_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $message ) {
				$message = sprintf(
 ...
 *
 * Default behavior is to trigger a user error if `DEBUG` is true.
 *
 ...
 * @param string $version     The version of Sync Site that deprecated the hook.
...
function _deprecated_hook( $hook, $version, $replacement = '', $message = '' ) {
	 ...
	 */
	if ( DEBUG && apply_filters( 'deprecated_hook_sync_trigger_error', true ) ) {
		$message = empty( $message ) ? '' : ' ' . $message;
 ...
 * @param string $version       The version of Sync Site where the message was added.
 */
function _doing_it_wrong( $function_name, $message, $version ) {
	 ...
	 * @param string $version       The version of Sync Site where the message was added.
	 */
	if ( DEBUG && apply_filters( 'doing_it_wrong_sync_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			if ( $version ) {
			...
			$message .= ' ' . __( 'Please see Debug issue.' );

			$message = sprintf(
				/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3: Sync Site version number. */
			...
			$message .= ' Please see Debug for more information.';
 ...
 *
 * Generates the message when `DEBUG` is true.
 *
 * @since 6.4.0
 *
 ...
 *                              before passing to this function to avoid being stripped {@see kses()}.
 * @param int    $error_level   Optional. The designated error type for this error.
 *                              Only works with E_USER family of constants. Default E_USER_NOTICE.
 */
function sync_trigger_error( $function_name, $message, $error_level = E_USER_NOTICE ) {

	// Bail out if DEBUG is not turned on.
	if ( ! DEBUG ) {
		return;
	}
	 ...
	 */
	do_action( 'sync_trigger_error_run', $function_name, $message, $error_level );
...
function validate_file( $file, $allowed_files = array() ) {
	...
	$allowed_files = array_map( 'normalize_path', $allowed_files );
...
function guess_url() {
	if ( defined( 'SITEURL' ) && '' !== SITEURL ) {
		...
		if ( str_contains( $_SERVER['REQUEST_URI'], 'admin' ) || str_contains( $_SERVER['REQUEST_URI'], 'login' ) ) {
			$path = preg_replace( '#/(admin/?.*|login\*)#i', '', $_SERVER['REQUEST_URI'] );
 ...
 */
function suspend_cache_addition( $suspend = null ) {
 ...
 * @return bool The current suspend setting.
 */
function suspend_cache_invalidation( $suspend = true ) {
	global $_suspend_cache_invalidation;

	$current_suspend                = $_suspend_cache_invalidation;
	$_suspend_cache_invalidation = $suspend;
...
function is_site_meta_supported() {
	...
	if ( false === $supported ) {
		$supported = $db->get_var( "SHOW TABLES LIKE '{$db->blogmeta}'" ) ? 1 : 0;
 ...
 */
function timezone_override_offset() {
	$timezone_string = get_option( 'timezone_string' );
	if ( ! $timezone_string ) {
		return false;
	}
 ...
 * @return int
 */
function _timezone_choice_usort_callback( $a, $b ) {
...
function timezone_choice( $selected_zone, $locale = null ) {
	...
	if ( ! $mo_loaded || $locale !== $locale_loaded ) {
		...
		$mofile        = LANG_DIR . '/continents-cities-' . $locale_loaded . '.mo';
	...
	usort( $zonen, '_timezone_choice_usort_callback' );
...
/**
 * Strips close comment and close php tags from file headers used by Sync Site.
 *
 * @since 2.8.0
 * @access private
 *
 ...
 * @global db $db Sync Site database abstraction object.
 */
function scheduled_delete() {
	global $db;

	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	$posts_to_delete = $db->get_results( $db->prepare( "SELECT post_id FROM $db->postmeta WHERE meta_key = '_trash_meta_time' AND meta_value < %d", $delete_timestamp ), ARRAY_A );
	...
	foreach ( (array) $posts_to_delete as $post ) {
		...
		if ( ! $del_post || 'trash' !== $del_post->post_status ) {
			delete_post_meta( $post_id, '_trash_meta_status' );
			delete_post_meta( $post_id, '_trash_meta_time' );
		} else {
			delete_post( $post_id );
		}
	}

	$comments_to_delete = $db->get_results( $db->prepare( "SELECT comment_id FROM $db->commentmeta WHERE meta_key = '_trash_meta_time' AND meta_value < %d", $delete_timestamp ), ARRAY_A );
	...
	foreach ( (array) $comments_to_delete as $comment ) {
		...
		if ( ! $del_comment || 'trash' !== $del_comment->comment_approved ) {
			delete_comment_meta( $comment_id, '_trash_meta_time' );
			delete_comment_meta( $comment_id, '_trash_meta_status' );
		} else {
 ...
 */
function find_hierarchy_loop( $callback, $start, $start_parent, $callback_args = array() ) {
	$override = is_null( $start_parent ) ? array() : array( $start => $start_parent );

	$arbitrary_loop_member = find_hierarchy_loop_tortoise_hare( $callback, $start, $override, $callback_args );
	if ( ! $arbitrary_loop_member ) {
		return array();
	}

	return find_hierarchy_loop_tortoise_hare( $callback, $arbitrary_loop_member, $override, $callback_args, true );
}
 ...
 *               $_return_loop
 */
function find_hierarchy_loop_tortoise_hare( $callback, $start, $override = array(), $callback_args = array(), $_return_loop = false ) {
	$tortoise        = $start;
	$hare            = $start;
	...
	foreach ( $trace as $call ) {
		if ( $skip_frames > 0 ) {
			...
			if ( in_array( $call['function'], array( 'do_action', 'apply_filters', 'do_action_ref_array', 'apply_filters_ref_array' ), true ) ) {
				...
				$caller[] = $call['function'] . "('" . str_replace( $truncate_paths, '', normalize_path( $filename ) ) . "')";
			} else {
...
function _get_non_cached_ids( $object_ids, $cache_group ) {
	...
	$cache_values   = cache_get_multiple( $object_ids, $cache_group );
 ...
 */
function sync_checkdate( $month, $day, $year, $source_date ) {
	/**
	 ...
	 */
	return apply_filters( 'sync_checkdate', checkdate( $month, $day, $year ), $source_date );
}

/**
 * Loads the auth check for monitoring whether the user is still logged in.
 *
 * Can be disabled with remove_action( 'admin_enqueue_scripts', 'auth_check_load' );
 *
 * This is disabled for certain screens where a login screen could cause an
 * inconvenient interruption. A filter called {@see 'auth_check_load'} can be used
 * for fine-grained control.
 *
 * @since 3.6.0
 */
function auth_check_load() {
	 ...
	 */
	if ( apply_filters( 'auth_check_load', $show, $screen ) ) {
		enqueue_style( 'auth-check' );
		enqueue_script( 'auth-check' );

		add_action( 'admin_print_footer_scripts', 'auth_check_html', 5 );
		add_action( 'print_footer_scripts', 'auth_check_html', 5 );
	}
}

/**
 * Outputs the HTML that shows the login dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
function auth_check_html() {
	$login_url      = login_url();
	 ...
	 */
	$same_domain = apply_filters( 'auth_check_same_domain', $same_domain );
	...
	<div id="auth-check">
	<button type="button" class="auth-check-close button-link"><span class="screen-reader-text">
		<?php
...
function _canonical_charset( $charset ) {
	 ...
	 * @todo Should Sync Site transform other potential charset equivalents, such as "latin1"?
	 */
	if (
...
function delete_file( $file ) {
	 ...
	 */
	$delete = apply_filters( 'delete_file', $file );
	if ( ! empty( $delete ) ) {
		@unlink( $delete );
	}
}
 ...
 */
function delete_file_from_directory( $file, $directory ) {
	if ( is_stream( $file ) ) {
		$real_file      = $file;
		$real_directory = $directory;
	} else {
		$real_file      = realpath( normalize_path( $file ) );
		$real_directory = realpath( normalize_path( $directory ) );
	}

	if ( false !== $real_file ) {
		$real_file = normalize_path( $real_file );
	}

	if ( false !== $real_directory ) {
		$real_directory = normalize_path( $real_directory );
	}
 ...
 * @since 4.3.0
 *
 * @global Post $post Global post object.
 */
function post_preview_js() {
	global $post;
	...
	<?php
	print_inline_script_tag( remove_surrounding_empty_script_tags( ob_get_clean() ) );
}
 ...
 */
function raise_memory_limit( $context = 'admin' ) {
	...
	$max_limit     = MAX_MEMORY_LIMIT;
	$max_limit_int = _convert_hr_to_bytes( $max_limit );
	$filtered_limit   = $max_limit;
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * The `MAX_MEMORY_LIMIT` constant specifically defines the maximum memory
			 ...
			 *
			 * @param int|string $filtered_limit The maximum Sync Site memory limit. Accepts an integer
		...
		case 'image':
			 ...
			 *                                   Default `MAX_MEMORY_LIMIT` or the original
		...
		case 'cron':
			/**
			 * Filters the memory limit allocated for Cron event processing.
			 *
			 * @since 6.3.0
			 *
			 * @param int|string $filtered_limit Maximum memory limit to allocate for Cron.
			 *                                   Default `MAX_MEMORY_LIMIT` or the original
			 *                                   php.ini `memory_limit`, whichever is higher.
			 *                                   Accepts an integer (bytes), or a shorthand string
			 *                                   notation, such as '256M'.
		...
		default:
			 ...
			 *                                   Default MAX_MEMORY_LIMIT` or the original php.ini `memory_limit`,
	...
	$filtered_limit_int = convert_hr_to_bytes( $filtered_limit );

	if ( -1 === $filtered_limit_int || ( $filtered_limit_int > $max_limit_int && $filtered_limit_int > $current_limit_int ) ) {
		if ( false !== ini_set( 'memory_limit', $filtered_limit ) ) {
			return $filtered_limit;
		} else {
			return false;
		}
	} elseif ( -1 === $max_limit_int || $max_limit_int > $current_limit_int ) {
		if ( false !== ini_set( 'memory_limit', $max_limit ) ) {
			return $max_limit;
		} else {
			return false;
		}
 ...
 */
function is_uuid( $uuid, $version = null ) {
 ...
 *
 * It is similar to `unique_id`, but each prefix has its own internal ID
 ...
 */
function unique_prefixed_id( $prefix = '' ) {
...
function cache_get_last_changed( $group ) {
	$last_changed = cache_get( 'last_changed', $group );

	if ( $last_changed ) {
		return $last_changed;
	}

	return cache_set_last_changed( $group );
}
...
function cache_set_last_changed( $group ) {
	$previous_time = cache_get( 'last_changed', $group );

	$time = microtime();

	cache_set( 'last_changed', $time, $group );
	 ...
	 */
	do_action( 'cache_set_last_changed', $group, $time, $previous_time );
 ...
 */
function site_admin_email_change_notification( $old_email, $new_email, $option_name ) {
	$send = true;
...
###SITEURL###'
	...
	// Get site name.
	$site_name = specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
 ...
 *
 * @since 4.9.6 Abstracted from `Community_Events::get_unsafe_client_ip()`.
 *
 ...
 */
function privacy_anonymize_ip( $ip_addr, $ipv6_fallback = false ) {
	if ( empty( $ip_addr ) ) {
		return '0.0.0.0';
	}
 ...
 */
function privacy_anonymize_data( $type, $data = '' ) {
	...
	switch ( $type ) {
		...
		case 'url':
			...
			break;
		case 'ip':
			$anonymous = privacy_anonymize_ip( $data );
			break;
		case 'date':
	 ...
	 */
	return apply_filters( 'privacy_anonymize_data', $anonymous, $type, $data );
}
 ...
 * @since 4.9.6
 *
 * @see privacy_exports_url
 *
 * @return string Exports directory.
 */
function privacy_exports_dir() {
	 ...
	 */
	return apply_filters( 'privacy_exports_dir', $exports_dir );
}
 ...
 * @since 4.9.6
 *
 * @see privacy_exports_dir
 *
 * @return string Exports directory URL.
 */
function privacy_exports_url() {
	$upload_dir  = upload_dir();
	 ...
	 */
	return apply_filters( 'privacy_exports_url', $exports_url );
}

/**
 * Schedules a `Cron` job to delete expired export files.
 *
 * @since 4.9.6
 */
function schedule_delete_old_privacy_export_files() {
	if ( installing() ) {
		return;
	}

	if ( ! next_scheduled( 'privacy_delete_old_export_files' ) ) {
		schedule_event( time(), 'hourly', 'privacy_delete_old_export_files' );
	}
}
 ...
 *
 * The export files are stored in `public/uploads`, and are therefore publicly
 ...
 * @since 4.9.6
 */
function privacy_delete_old_export_files() {
	$exports_dir = privacy_exports_dir();
	 ...
	 */
	$expiration = apply_filters( 'privacy_export_expiration', 3 * DAY_IN_SECONDS );
 ...
 *
 * This URL can be overridden by specifying an environment variable `UPDATE_PHP_URL` or by using the
 * {@see 'update_php_url'} filter. Providing an empty string is not allowed and will result in the
 ...
 */
function get_update_php_url() {
	$default_url = get_default_update_php_url();

	$update_url = $default_url;
	if ( false !== getenv( 'UPDATE_PHP_URL' ) ) {
		$update_url = getenv( 'UPDATE_PHP_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'update_php_url', $update_url );
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see get_update_php_url()} when relying on the URL.
 ...
 */
function get_default_update_php_url() {
	return _x( SYNC_UPDATE_INFO_URL, 'localized PHP upgrade information page' );
}

/**
 * Prints the default annotation for the web host altering the "Update PHP" page URL.
 *
 * This function is to be used after {@see get_update_php_url()} to display a consistent
 ...
 * @return string|void
 */
function update_php_annotation( $before = '<p class="description">', $after = '</p>', $display = true ) {
	$annotation = get_update_php_annotation();
 ...
 *
 * This function is to be used after {@see get_update_php_url()} to return a consistent
 * annotation if the web host has altered the default "Update PHP" page URL.
 *
 * @since 5.2.0
 *
 ...
 */
function get_update_php_annotation() {
	$update_url  = get_update_php_url();
	$default_url = get_default_update_php_url();
	...
	$annotation = __( 'This resource is provided by your web host, and is specific to your site.' );
 ...
 *
 * A URL will only be returned if the `DIRECT_UPDATE_PHP_URL` environment variable is specified or
 * by using the {@see 'direct_php_update_url'} filter. This allows hosts to send users directly to
 ...
 */
function get_direct_php_update_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'DIRECT_UPDATE_PHP_URL' ) ) {
		$direct_update_url = getenv( 'DIRECT_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$direct_update_url = apply_filters( 'direct_php_update_url', $direct_update_url );
 ...
 *
 * The button is only displayed if a URL is returned by `get_direct_php_update_url()`.
 *
 * @since 5.1.1
 */
function direct_php_update_button() {
	$direct_update_url = get_direct_php_update_url();
 ...
 *
 * This URL can be overridden by specifying an environment variable `UPDATE_HTTPS_URL` or by using the
 * {@see 'update_https_url'} filter. Providing an empty string is not allowed and will result in the
 * default URL being used. Furthermore the page the URL links to should preferably be localized in the
 * site language.
 *
 * @since 5.7.0
 ...
 */
function get_update_https_url() {
	$default_url = get_default_update_https_url();

	$update_url = $default_url;
	if ( false !== getenv( 'UPDATE_HTTPS_URL' ) ) {
		$update_url = getenv( 'UPDATE_HTTPS_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'update_https_url', $update_url );
	if ( empty( $update_url ) ) {
		$update_url = $default_url;
	}
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see get_update_https_url()} when relying on the URL.
 * This function does not allow modifying the returned URL, and is only used to compare the actually used URL with the
 * default one.
 *
 * @since 5.7.0
 ...
 */
function get_default_update_https_url() {
	/* translators: Documentation explaining HTTPS and why it should be used. */
	return __( SYNC_UPDATE_HELP_URL );
}

/**
 * Gets the URL for directly updating the site to use HTTPS.
 *
 * A URL will only be returned if the `DIRECT_UPDATE_HTTPS_URL` environment variable is specified or
 * by using the {@see 'direct_update_https_url'} filter. This allows hosts to send users directly to
 ...
 */
function get_direct_update_https_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'DIRECT_UPDATE_HTTPS_URL' ) ) {
		$direct_update_url = getenv( 'DIRECT_UPDATE_HTTPS_URL' );
	}
	 ...
	 * @param string $direct_update_url URL for directly updating PHP.
	 */
	$direct_update_url = apply_filters( 'direct_update_https_url', $direct_update_url );
 ...
 *                                   The timeout is global and is measured from the moment Sync Site started to load.
 ...
 *                                            Sync Site started to load. Defaults to the value of
...
function recurse_dirsize( $directory, $exclude = null, $max_execution_time = null, &$directory_cache = null ) {
			...
			while ( ( $file = readdir( $handle ) ) !== false ) {
				...
				if ( '.' !== $file && '..' !== $file ) {
					...
					if ( $max_execution_time > 0 &&
						( microtime( true ) - START_TIMESTAMP ) > $max_execution_time
					) {
	...
	if ( $save_cache ) {
		$expiration = ( using_ext_object_cache() ) ? 0 : 10 * YEAR_IN_SECONDS;
...
/**
 * Checks compatibility with the current Sync Site version.
 *
 * @since 5.2.0
 *
 * @global string $version The Sync Site version string.
 *
 * @param string $required Minimum required Sync Site version.
 * @return bool True if required version is compatible or empty, false if not.
 */
function is_version_compatible( $required ) {
	global $version;
 ...
 */
function fuzzy_number_match( $expected, $actual, $precision = 1 ) {
 ...
 */
function get_admin_notice( $message, $args = array() ) {
	$defaults = array(
		'type'               => '',
		'dismissible'        => false,
		'id'                 => '',
	 ...
	 */
	$args       = apply_filters( 'admin_notice_args', $args, $message );
	$id         = '';
	$classes    = 'notice';
	$attributes = '';
	 ...
	 * @param array  $args    The arguments for the admin notice.
	 */
	return apply_filters( 'admin_notice_markup', $markup, $message, $args );
}
 ...
 * }
 */
function admin_notice( $message, $args = array() ) {
	/**
	 ...
	 * @param array  $args    The arguments for the admin notice.
	 */
	do_action( 'admin_notice', $message, $args );

	echo kses_post( get_admin_notice( $message, $args ) );
}
