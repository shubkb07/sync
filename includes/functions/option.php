...
function get_option( $option, $default_value = false ) {
	...
	if ( isset( $deprecated_keys[ $option ] ) && ! installing() ) {
	...
	if ( ! installing() ) {
		...
		if ( isset( $alloptions[ $option ] ) ) {
			...
			$value = cache_get( $option, 'options' );

			if ( false === $value ) {
				// Prevent non-existent options from triggering multiple queries.
				$notoptions = cache_get( 'notoptions', 'options' );

				// Prevent non-existent `notoptions` key from triggering multiple key lookups.
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
					cache_set( 'notoptions', $notoptions, 'options' );
				...
				if ( is_object( $row ) ) {
					...
					cache_add( $option, $value, 'options' );
				} else { // Option does not exist, so we must cache its non-existence.
					$notoptions[ $option ] = true;
					cache_set( 'notoptions', $notoptions, 'options' );

					/** This filter is documented in includes/functions/option.php */
					return apply_filters( "default_option_{$option}", $default_value, $option, $passed_default );
				}
			}
		}
 ...
 */
function prime_option_caches( $options ) {
	global $db;

	$alloptions     = load_alloptions();
	$cached_options = cache_get_multiple( $options, 'options' );
	$notoptions     = cache_get( 'notoptions', 'options' );
	if ( ! is_array( $notoptions ) ) {
		$notoptions = array();
	}
	...
	$results = $db->get_results(
		$db->prepare(
			sprintf(
				"SELECT option_name, option_value FROM $db->options WHERE option_name IN (%s)",
	...
	cache_set_multiple( $options_found, 'options' );
 ...
 */
function prime_option_caches_by_group( $option_group ) {
	global $new_allowed_options;

	if ( isset( $new_allowed_options[ $option_group ] ) ) {
		prime_option_caches( $new_allowed_options[ $option_group ] );
	}
}
 ...
 *               was updated.
 */
function set_option_autoload_values( array $options ) {
	global $db;
	...
	foreach ( $options as $option => $autoload ) {
		protect_special_option( $option ); // Ensure only valid options can be passed.
	 ...
	 */
	$options_to_update = $db->get_col( $db->prepare( "SELECT option_name FROM $db->options $where", $where_args ) );
	...
	foreach ( $grouped_options as $autoload => $options ) {
		...
		$success = $db->query(
			$db->prepare(
				"UPDATE $db->options SET autoload = %s WHERE option_name IN (" . implode( ',', array_fill( 0, count( $grouped_options[ $autoload ] ), '%s' ) ) . ')',
				array_merge(
					array( $autoload ),
	...
	if ( $grouped_options['on'] ) {
		cache_delete_multiple( $grouped_options['on'], 'options' );
		cache_delete( 'alloptions', 'options' );
	} elseif ( $grouped_options['off'] ) {
		$alloptions = load_alloptions( true );
		...
		cache_set( 'alloptions', $alloptions, 'options' );
	}
 ...
 *
 * This is a wrapper for {@see set_option_autoload_values()}, which can be used to set different autoload values for
 * each option at once.
 *
 * @since 6.4.0
 *
 * @see set_option_autoload_values()
 *
 * @param string[]    $options  List of option names. Expected to not be SQL-escaped.
 * @param string|bool $autoload Autoload value to control whether to load the options when Sync Site starts up.
 *                              Accepts 'yes'|true to enable or 'no'|false to disable.
 * @return array Associative array of all provided $options as keys and boolean values for whether their autoload value
 *               was updated.
 */
function set_options_autoload( array $options, $autoload ) {
	return set_option_autoload_values(
 ...
 *
 * This is a wrapper for {@see set_option_autoload_values()}, which can be used to set the autoload value for
 ...
 * @param string|bool $autoload Autoload value to control whether to load the option when Sync Site starts up.
 *                              Accepts 'yes'|true to enable or 'no'|false to disable.
 * @return bool True if the autoload value was modified, false otherwise.
 */
function set_option_autoload( $option, $autoload ) {
	$result = set_option_autoload_values( array( $option => $autoload ) );
...
/**
 * Protects Sync Site special option from being modified.
 *
 ...
 * @param string $option Option name.
 */
function protect_special_option( $option ) {
	if ( 'alloptions' === $option || 'notoptions' === $option ) {
		sync_die(
			sprintf(
				/* translators: %s: Option name. */
				__( '%s is a protected Site option and may not be modified' ),
				esc_html( $option )
			)
		);
	}
 ...
 */
function load_alloptions( $force_cache = false ) {
	global $db;
	 ...
	 * load_alloptions(), returning that value instead.
	 *
	 * @since 6.2.0
	 *
	 ...
	 */
	$alloptions = apply_filters( 'pre_load_alloptions', null, $force_cache );
	if ( is_array( $alloptions ) ) {
		return $alloptions;
	}

	if ( ! installing() || ! is_multisite() ) {
		$alloptions = cache_get( 'alloptions', 'options', $force_cache );
	} else {
		$alloptions = false;
	}
	...
	if ( ! $alloptions ) {
		$suppress      = $db->suppress_errors();
		$alloptions_db = $db->get_results( "SELECT option_name, option_value FROM $db->options WHERE autoload IN ( '" . implode( "', '", esc_sql( autoload_values_to_autoload() ) ) . "' )" );
		...
		if ( ! installing() || ! is_multisite() ) {
			...
			cache_add( 'alloptions', $alloptions, 'options' );
		}
	}
 ...
 *
 * If site is not multisite, then call prime_option_caches().
 *
 * @since 6.6.0
 *
 * @see prime_network_option_caches()
 *
 * @param string[] $options An array of option names to be loaded.
 */
function prime_site_option_caches( array $options ) {
	prime_network_option_caches( null, $options );
}
 ...
 * @param string[] $options    An array of option names to be loaded.
 */
function prime_network_option_caches( $network_id, array $options ) {
	global $db;
	...
	$cached_options = cache_get_multiple( array_values( $cache_keys ), $cache_group );

	$notoptions_key = "$network_id:notoptions";
	$notoptions     = cache_get( $notoptions_key, $cache_group );
	...
	$results      = $db->get_results(
		$db->prepare(
			sprintf(
				"SELECT meta_key, meta_value FROM $db->sitemeta WHERE meta_key IN (%s) AND site_id = %s",
				implode( ',', array_fill( 0, count( $options_to_prime ), '%s' ) ),
				'%d'
			),
			$query_args
	...
	cache_set_multiple( $data, $cache_group );
	// If all options were found, no need to update `notoptions` cache.
	if ( count( $options_found ) === count( $options_to_prime ) ) {
		return;
	}
	...
	// Only update the cache if it was modified.
	if ( $update_notoptions ) {
		cache_set( $notoptions_key, $notoptions, $cache_group );
	}
}
 ...
 * @since 6.6.0 Uses prime_network_option_caches().
 *
 * @param int $network_id Optional. Network ID of network for which to prime network options cache. Defaults to current network.
 */
function load_core_site_options( $network_id = null ) {
	if ( ! is_multisite() || installing() ) {
		return;
	}
	$core_options = array( 'site_name', 'siteurl', 'active_sitewide_plugins', '_site_transient_timeout_theme_roots', '_site_transient_theme_roots', 'site_admins', 'can_compress_scripts', 'global_terms_enabled', 'ms_files_rewriting', 'LANG' );

	prime_network_option_caches( $network_id, $core_options );
}
 ...
 * @param bool|null $autoload Optional. Whether to load the option when Sync Site starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is set,
 *                            to leave the decision up to default heuristics in Sync Site.
 ...
 *                            For non-existent options, the default is null, which means Sync Site will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	...
	if ( null !== $autoload ) {
		$update_args['autoload'] = determine_option_autoload_value( $option, $value, $serialized_value, $autoload );
	} else {
		// Retrieve the current autoload value to reevaluate it in case it was set automatically.
		$raw_autoload = $db->get_var( $db->prepare( "SELECT autoload FROM $db->options WHERE option_name = %s LIMIT 1", $option ) );
		$allow_values = array( 'auto-on', 'auto-off', 'auto' );
		if ( in_array( $raw_autoload, $allow_values, true ) ) {
			$autoload = determine_option_autoload_value( $option, $value, $serialized_value, $autoload );
	...
	if ( ! installing() ) {
		if ( ! isset( $update_args['autoload'] ) ) {
			...
			if ( isset( $alloptions[ $option ] ) ) {
				...
				cache_set( $option, $serialized_value, 'options' );
			}
		} elseif ( in_array( $update_args['autoload'], autoload_values_to_autoload(), true ) ) {
			// Delete the individual cache, then set in alloptions cache.
			cache_delete( $option, 'options' );
 ...
 * aren't adding a protected Sync Site option. Care should be taken to not name
 ...
 * @param bool|null $autoload   Optional. Whether to load the option when Sync Site starts up.
 *                              Accepts a boolean, or `null` to leave the decision up to default heuristics in Sync Site.
 *                              For backward compatibility 'yes' and 'no' are also accepted.
 *                              Autoloading too many options can lead to performance problems, especially if the
 *                              options are not frequently used. For options which are accessed across several places
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means Sync Site will determine the autoload value.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = null ) {
	...
	if ( ! installing() ) {
		if ( in_array( $autoload, autoload_values_to_autoload(), true ) ) {
			$alloptions            = load_alloptions( true );
			$alloptions[ $option ] = $serialized_value;
			cache_set( 'alloptions', $alloptions, 'options' );
		} else {
	...
	$notoptions = cache_get( 'notoptions', 'options' ); // Yes, again... we need it to be fresh.
...
/**
 * Removes an option by name. Prevents removal of protected Sync Site options.
 *
 * @since 1.2.0
 *
 * @global db $db Sync Site database abstraction object.
...
function delete_option( $option ) {
	...
	if ( ! installing() ) {
		if ( in_array( $row->autoload, autoload_values_to_autoload(), true ) ) {
			$alloptions = load_alloptions( true );
 ...
 */
function determine_option_autoload_value( $option, $value, $serialized_value, $autoload ) {
	 ...
	 */
	$autoload = apply_filters( 'default_autoload_value', null, $option, $value, $serialized_value );
	if ( is_bool( $autoload ) ) {
 ...
 */
function filter_default_autoload_value_via_option_size( $autoload, $option, $value, $serialized_value ) {
	/**
	 ...
	 * @param string $option          The name of the option.
	 */
	$max_option_size = (int) apply_filters( 'max_autoloaded_option_size', 150000, $option );
...
function delete_transient( $transient ) {
	...
	if ( using_ext_object_cache() || installing() ) {
		$result = cache_delete( $transient, 'transient' );
	} else {
...
function get_transient( $transient ) {
	...
	if ( using_ext_object_cache() || installing() ) {
		$value = cache_get( $transient, 'transient' );
	} else {
		...
		if ( ! installing() ) {
			...
			if ( ! isset( $alloptions[ $transient_option ] ) ) {
				...
				prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( using_ext_object_cache() || installing() ) {
		$result = cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
		$transient_option  = '_transient_' . $transient;
		prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function delete_expired_transients( $force_db = false ) {
	...
	if ( ! $force_db && using_ext_object_cache() ) {
		return;
	}
...
function user_settings() {
	...
	setcookie( 'settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	setcookie( 'settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
...
function set_user_setting( $name, $value ) {
	...
	return set_all_user_settings( $all_user_settings );
}
 ...
 *                   Null if the current user is not a member of the site.
 */
function set_all_user_settings( $user_settings ) {
	global $_updated_user_settings;
...
function get_network_option( $network_id, $option, $default_value = false ) {
	...
	// Prevent non-existent options from triggering multiple queries.
	$notoptions_key = "$network_id:notoptions";
	$notoptions     = cache_get( $notoptions_key, 'site-options' );
	...
	if ( ! is_multisite() ) {
		...
		$value     = cache_get( $cache_key, 'site-options' );
		...
		if ( ! isset( $value ) || false === $value ) {
			...
			if ( is_object( $row ) ) {
				...
				cache_set( $cache_key, $value, 'site-options' );
			} else {
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
				}
				...
				$notoptions[ $option ] = true;
				cache_set( $notoptions_key, $notoptions, 'site-options' );
...
function add_network_option( $network_id, $option, $value ) {
	...
	if ( ! is_multisite() ) {
		...
		// This option exists now.
		$notoptions = cache_get( $notoptions_key, 'site-options' ); // Yes, again... we need it to be fresh.
...
function delete_network_option( $network_id, $option ) {
	...
	if ( ! is_multisite() ) {
		...
		$cache_key = "$network_id:$option";
		cache_delete( $cache_key, 'site-options' );
...
function delete_site_transient( $transient ) {
	...
	if ( using_ext_object_cache() || installing() ) {
		$result = cache_delete( $transient, 'site-transient' );
	} else {
...
function get_site_transient( $transient ) {
	...
	if ( using_ext_object_cache() || installing() ) {
		$value = cache_get( $transient, 'site-transient' );
	} else {
		...
		if ( ! in_array( $transient, $no_timeout, true ) ) {
			...
			prime_site_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_site_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( using_ext_object_cache() || installing() ) {
		$result = cache_set( $transient, $value, 'site-transient', $expiration );
	} else {
		$transient_timeout = '_site_transient_timeout_' . $transient;
		$option            = '_site_transient_' . $transient;
		prime_site_option_caches( array( $option, $transient_timeout ) );
...
/**
 * Registers default settings available in Sync Site.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in Sync Site.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'Sync locale code.' ),
			'default'      => 'en_US',
		)
	);
 ...
 *
 * @global array $new_allowed_options
 * @global array $registered_settings
 *
...
function register_setting( $option_group, $option_name, $args = array() ) {
	global $new_allowed_options, $registered_settings;
	...
	$registered_settings[ $option_name ] = $args;
}
...
function unregister_setting( $option_group, $option_name, $deprecated = '' ) {
	...
	if ( isset( $registered_settings[ $option_name ] ) ) {
		...
		if ( ! empty( $registered_settings[ $option_name ]['sanitize_callback'] ) ) {
			remove_filter( "sanitize_option_{$option_name}", $registered_settings[ $option_name ]['sanitize_callback'] );
		}

		// Remove the default filter if a default was provided during registration.
		if ( array_key_exists( 'default', $registered_settings[ $option_name ] ) ) {
		...
		unset( $registered_settings[ $option_name ] );
	}
}
 ...
 */
function autoload_values_to_autoload() {
	 ...
	 */
	$filtered_values = apply_filters( 'autoload_values_to_autoload', $autoload_values );
