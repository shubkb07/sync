 ...
 */
 ...
 require_once LIB . '/pomo/mo.php';
 require_once CLASSES . '/l10n/class-translation-controller.php';
 require_once CLASSES . '/l10n/class-translations.php';
 require_once CLASSES . '/l10n/class-translation-file.php';
 require_once CLASSES . '/l10n/class-translation-file-mo.php';
 require_once CLASSES . '/l10n/class-translation-file-php.php';
 require_once CLASSES . '/l10n/class-textdomain-registry.php';
 require_once CLASSES . '/l10n/class-locale.php';
 require_once CLASSES . '/l10n/class-locale-switcher.php';
 ...
 * @global string $local_package Locale code of the package.
 *
 * @return string The locale of the blog or from the {@see 'locale'} hook.
 */
function get_locale() {
	global $locale, $local_package;

	if ( isset( $locale ) ) {
		/** This filter is documented in includes/functions/l10n.php */
		return apply_filters( 'locale', $locale );
	}

	if ( isset( $local_package ) ) {
		$locale = $local_package;
	}

	// LANG was defined in sync-config.
	if ( defined( 'LANG' ) ) {
		$locale = LANG;
	}
	...
	/**
	 * Filters the locale ID of the Sync installation.
	 *
	 * @since 1.5.0
	 *
 ...
 * @since 4.7.0
 *
 * @param int|User $user User's ID or a User object. Defaults to current user.
 * @return string The locale of the user.
 */
function get_user_locale( $user = 0 ) {
	$user_object = false;

	if ( 0 === $user && function_exists( 'get_current_user' ) ) {
...
function determine_locale() {
	...
	if (
		isset( $GLOBALS['pagenow'] ) && 'login' === $GLOBALS['pagenow'] &&
		( ! empty( $_GET['lang'] ) || ! empty( $_COOKIE['lang'] ) )
	) {
		if ( ! empty( $_GET['lang'] ) ) {
			$determined_locale = sanitize_locale_name( $_GET['lang'] );
		} else {
			$determined_locale = sanitize_locale_name( $_COOKIE['lang'] );
		}
	} elseif (
		is_admin() ||
		( isset( $_GET['_locale'] ) && 'user' === $_GET['_locale'] && is_json_request() )
	) {
		$determined_locale = get_user_locale();
	} elseif (
		( ! empty( $_REQUEST['language'] ) || isset( $GLOBALS['local_package'] ) )
		&& installing()
	) {
		if ( ! empty( $_REQUEST['language'] ) ) {
			$determined_locale = sanitize_locale_name( $_REQUEST['language'] );
		} else {
			$determined_locale = $GLOBALS['local_package'];
		}
	}
 ...
 * @global Textdomain_Registry $textdomain_registry Sync Textdomain Registry.
 *
 * @param string $domain Text domain. Unique identifier for retrieving translated strings.
...
function load_textdomain( $domain, $mofile, $locale = null ) {
	/** @var Textdomain_Registry $textdomain_registry */
	global $l10n, $l10n_unloaded, $textdomain_registry;
	...
	$i18n_controller = Translation_Controller::get_instance();
	...
	foreach ( $translation_files as $file ) {
		...
		if ( $success ) {
			...
			$l10n[ $domain ] = new Sync_Translations( $i18n_controller, $domain );

			$textdomain_registry->set( $domain, $locale, dirname( $file ) );
...
function unload_textdomain( $domain, $reloadable = false ) {
	...
	if ( ! $reloadable ) {
		Translation_Controller::get_instance()->unload_textdomain( $domain );
	}
 ...
 *
 * Loads the .mo file in LANG_DIR constant path from Sync root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', LANG_DIR . "/$locale.mo", $locale );

	if ( ( is_multisite() || ( defined( 'INSTALLING_NETWORK' ) && INSTALLING_NETWORK ) ) && ! file_exists( LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', LANG_DIR . "/ms-$locale.mo", $locale );
		return $return;
	}

	if ( is_admin() || installing() || ( defined( 'REPAIRING' ) && REPAIRING ) ) {
		load_textdomain( 'default', LANG_DIR . "/admin-$locale.mo", $locale );
	}

	if ( is_network_admin() || ( defined( 'INSTALLING_NETWORK' ) && INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', LANG_DIR . "/admin-network-$locale.mo", $locale );
	}
 ...
 *                                      Default false.
 * @param string|false $plugin_rel_path Optional. Relative path to PLUGIN_DIR where the .mo file resides.
 *                                      Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	if ( load_textdomain( $domain, LANG_DIR . '/plugins/' . $mofile, $locale ) ) {
		return true;
	}
	...
	$textdomain_registry->set_custom_path( $domain, $path );
...
function load_theme_textdomain( $domain, $path = false ) {
	...
	// Try to load from the languages directory first.
	if ( load_textdomain( $domain, LANG_DIR . '/themes/' . $mofile, $locale ) ) {
		return true;
	}
...
function load_script_textdomain( $handle, $domain = 'default', $path = '' ) {
	...
	$src_url     = sync_parse_url( $src );
	$content_url = sync_parse_url( content_url() );
	$plugins_url = sync_parse_url( plugins_url() );
	$site_url    = sync_parse_url( site_url() );
	...
	if (
		...
		$languages_path = LANG_DIR . '/plugins';
		...
		if ( ! isset( $site_url['path'] ) ) {
			...
			// Make the src relative to the Sync Site root.
	...
	// If the source is not from Sync Site.
	if ( false === $relative ) {
 ...
 * the translation file from `public/languages`, removing the need
...
function _load_textdomain_just_in_time( $domain ) {
	/** @var Textdomain_Registry $textdomain_registry */
	global $l10n_unloaded, $textdomain_registry;
	...
	if ( ! $textdomain_registry->has( $domain ) ) {
		return false;
	}

	$locale = determine_locale();
	$path   = $textdomain_registry->get( $domain, $locale );
	if ( ! $path ) {
		return false;
	}
	// Themes with their language directory outside of LANG_DIR have a different file name.
...
/**
 * Returns the Sync_Translations instance for a text domain.
 *
 * If there isn't one, returns empty Sync_Translations instance.
 *
 * @since 2.8.0
 *
 * @global MO[] $l10n An array of all currently loaded text domains.
 *
 * @param string $domain Text domain. Unique identifier for retrieving translated strings.
 * @return Sync_Translations|NOOP_Translations A Sync_Translations instance.
 */
...
function get_available_languages( $dir = null ) {
	...
	$path       = is_null( $dir ) ? LANG_DIR : $dir;
	$lang_files = $textdomain_registry->get_language_files_from_path( $path );
 ...
 *
 * Looks in the public/languages directory for translations of
 * plugins or themes.
 *
 * @since 3.7.0
 *
...
function get_installed_translations( $type ) {
	...
	$dir = 'core' === $type ? LANG_DIR : LANG_DIR . "/$type";

	if ( ! is_dir( $dir ) ) {
		return array();
	}

	$files = $textdomain_registry->get_language_files_from_path( $dir );
	if ( ! $files ) {
		return array();
	}
	...
	foreach ( $files as $file ) {
		...
		if ( str_ends_with( $file, '.mo' ) ) {
			...
			$language_data[ $textdomain ][ $language ] = get_pomo_file_data( $pofile );
		} else {
			...
			$language_data[ $textdomain ][ $language ] = get_l10n_php_file_data( $file );
		}
	}
 ...
 */
function get_pomo_file_data( $po_file ) {
 ...
 */
function get_l10n_php_file_data( $php_file ) {
 ...
 * @see get_available_translations()
 *
 ...
 * @since 3.0.0
 *
 * @global Sync_Locale $locale Sync date and time locale object.
 *
 * @return bool Whether locale is RTL.
 */
function is_rtl() {
	global $locale;
	if ( ! ( $locale instanceof Sync_Locale ) ) {
		return false;
	}
	return $locale->is_rtl();
}
 ...
 * @since 4.7.0
 *
 * @global Sync_Locale_Switcher $locale_switcher Sync locale switcher object.
 *
 * @param string $locale The locale.
 * @return bool True on success, false on failure.
 */
function switch_to_locale( $locale ) {
	/* @var Sync_Locale_Switcher $locale_switcher */
	global $locale_switcher;

	if ( ! $locale_switcher ) {
		return false;
	}

	return $locale_switcher->switch_to_locale( $locale );
}
...
function switch_to_user_locale( $user_id ) {
	...
	return $locale_switcher->switch_to_user_locale( $user_id );
}
...
function restore_previous_locale() {
	...
	return $locale_switcher->restore_previous_locale();
}
...
function restore_current_locale() {
	...
	return $locale_switcher->restore_current_locale();
}
...
function is_locale_switched() {
	...
	return $locale_switcher->is_switched();
}
...
function get_list_item_separator() {
	...
	if ( ! ( $locale instanceof Sync_Locale ) ) {
		// Default value of Sync_Locale::get_list_item_separator().
		/* translators: Used between list items, there is a space after the comma. */
		return __( ', ' );
	}

	return $locale->get_list_item_separator();
}
...
function get_word_count_type() {
	...
	if ( ! ( $locale instanceof Sync_Locale ) ) {
		// Default value of Sync_Locale::get_word_count_type().
		return 'words';
	}

	return $locale->get_word_count_type();
}
