 ...
 * @return Post|null Post object on success, null on failure
 */
 ...
 * @return true|Sync_Error True on success or a Sync_Error object if the user doesn't exist
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new Sync_Error( 'user_does_not_exist', __( 'The requested user does not exist.' ) );
	}
	 ...
	 * @since 4.9.0
	 *
	 * @param true|Sync_Error $retval  True if the user should be added to the site, error
	 *                               object otherwise.
	...
	if ( true !== $can_add_user ) {
		...
		if ( is_error( $can_add_user ) ) {
			return $can_add_user;
		}

		return new Sync_Error( 'user_cannot_be_added', __( 'User cannot be added to this site.' ) );
	}
	...
	cache_delete( $blog_id . '_user_count', 'blog-details' );
...
function remove_user_from_blog( $user_id, $blog_id = 0, $reassign = 0 ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new Sync_Error( 'user_does_not_exist', __( 'That user does not exist.' ) );
	}
...
function get_blog_id_from_url( $domain, $path = '/' ) {
	...
	$id     = cache_get( md5( $domain . $path ), 'blog-id-cache' );
	...
	if ( ! $id ) {
		cache_set( md5( $domain . $path ), -1, 'blog-id-cache' );
		return 0;
	}

	cache_set( md5( $domain . $path ), $id, 'blog-id-cache' );
 ...
 *     @type Sync_Error $errors        Sync_Error object containing any errors found.
 * }
 */
function mu_validate_user_signup( $user_name, $user_email ) {
	...
	/** This filter is documented in includes/functions/user.php */
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from Sync Site. Please use another email provider.' ) );
	}
 ...
 * the way that Sync Site validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global db   $db   Sync Site database abstraction object.
 ...
 *     @type Sync_Error       $errors     Sync_Error containing any errors found.
 * }
 */
...
function mu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	...
	$key = substr( md5( time() . sync_rand() . $domain ), 0, 16 );
...
function mu_signup_user( $user, $user_email, $meta = array() ) {
	...
	$user_email = sanitize_email( $user_email );
	$key        = substr( md5( time() . sync_rand() . $user_email ), 0, 16 );
...
function mu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	if ( '' === $admin_email ) {
		$admin_email = 'support@' . sync_parse_url( network_home_url(), PHP_URL_HOST );
	}

	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : 'Sync Site';
	...
	$message = sprintf(
		 ...
		 *
		 * Content should be formatted for transmission via sync_mail().
		 *
		 * @since MU (3.0.0)
		 *
	...
	sync_mail( $user_email, specialchars_decode( $subject ), $message, $message_headers );
 ...
 * @return array|Sync_Error An array containing information about the activated user and/or blog.
 */
function mu_activate_signup( $key ) {
	...
	if ( empty( $signup ) ) {
		return new Sync_Error( 'invalid_key', __( 'Invalid activation key.' ) );
	}

	if ( $signup->active ) {
		if ( empty( $signup->domain ) ) {
			return new Sync_Error( 'already_active', __( 'The user is already active.' ), $signup );
		} else {
			return new Sync_Error( 'already_active', __( 'The site is already active.' ), $signup );
		}
	}

	$meta     = maybe_unserialize( $signup->meta );
	$password = generate_password( 12, false );
	...
	if ( ! $user_id ) {
		return new Sync_Error( 'create_user', __( 'Could not create user' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new Sync_Error( 'user_already_exists', __( 'That username is already activated.' ), $signup );
		}
 ...
 */
function delete_signup_on_user_delete( $id, $reassign, $user ) {
	global $db;
...
function mu_create_user( $user_name, $password, $email ) {
	...
	$user_id = create_user( $user_name, $password, $email );
	if ( is_error( $user_id ) ) {
		return false;
	}
 ...
 *                           Default 1.
 * @return int|Sync_Error Returns Sync_Error object on failure, the new site ID on success.
 */
function mu_create_blog( $domain, $path, $title, $user_id, $options = array(), $network_id = 1 ) {
	...
	if ( domain_exists( $domain, $path, $network_id ) ) {
		return new Sync_Error( 'blog_taken', __( 'Sorry, that site already exists!' ) );
	}
	...
	// Data to pass to initialize_site().
	...
	$blog_id = insert_site( array_merge( $site_data, $site_initialization_data ) );
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 $blog_id now supports input from the {@see 'initialize_site'} action.
 *
 * @param Site|int $blog_id    The new site's object or ID.
...
Disable these notifications: %4$s'
	...
	sync_mail( $email, sprintf( __( 'New Site Registration: %s' ), $siteurl ), $msg );
...
Disable these notifications: %3$s'
	...
	sync_mail( $email, sprintf( __( 'New User Registration: %s' ), $user->user_login ), $msg );
...
--The Team @ SITE_NAME'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'Sync Site';
	}
	 ...
	 */
	$subject = apply_filters( 'update_welcome_subject', sprintf( $subject, $current_network->site_name, unslash( $title ) ) );

	sync_mail( $user->user_email, specialchars_decode( $subject ), $message, $message_headers );
...
function mu_new_site_admin_notification( $site_id, $user_id ) {
	 ...
	 * @param Site $site Site object of the new site.
	 * @param User $user User object of the administrator of the new site.
	 */
...
Name: %3$s'
	...
	sync_mail(
		$new_site_email['to'],
		specialchars_decode( $new_site_email['subject'] ),
 ...
 *
 * @return Network The current network.
 */
 ...
 *
 * Sync Site ships with a set of allowed upload filetypes,
 * which is defined in includes/functions/functions.php in
 ...
 *
 * Sync Site MS stores a blog's post count as an option so as
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 Parameters now support input from the {@see 'initialize_site'} action.
 *
 * @global db $db Sync Site database abstraction object.
 *
 * @param Site|int $blog_id The new site's object or ID.
	...
	if ( $user ) {
		$db->insert(
			...
			array(
				...
				'IP'              => preg_replace( '/[^0-9., ]/', '', unslash( $_SERVER['REMOTE_ADDR'] ) ),
				'blog_id'         => $blog_id,
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( 'IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}
...
function signup_nonce_fields() {
	...
	nonce_field( 'signup_form_' . $id, '_signup_form', false );
}
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'signup' ) ) {
		return $result;
	}

	if ( ! verify_nonce( $_POST['_signup_form'], 'signup_form_' . $_POST['signup_form_id'] ) ) {
 ...
 * added, as when a user is invited through the regular Site Add User interface.
 *
 * @since MU (3.0.0)
 */
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is_error( add_existing_user_to_blog( $details ) ) ) {
		sync_die(
			sprintf(
 ...
 * }
 * @return true|Sync_Error|void True on success or a Sync_Error object if the user doesn't exist
...
function add_existing_user_to_blog( $details = false ) {
	if ( is_array( $details ) ) {
		 ...
		 *
		 * @param int           $user_id User ID.
		 * @param true|Sync_Error $result  True on success or a Sync_Error object if the user doesn't exist
		 *                               or could not be added.
		 */
 ...
 * @since 3.1.0
 */
function schedule_update_network_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! next_scheduled( 'update_network_counts' ) && ! installing() ) {
		schedule_event( time(), 'twicedaily', 'update_network_counts' );
	}
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function update_network_counts( $network_id = null ) {
	update_network_user_counts( $network_id );
	update_network_site_counts( $network_id );
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function maybe_update_network_site_counts( $network_id = null ) {
	$is_small_network = ! is_large_network( 'sites', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function maybe_update_network_user_counts( $network_id = null ) {
	$is_small_network = ! is_large_network( 'users', $network_id );

	/** This filter is documented in includes/functions/ms-functions.php */
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function update_network_site_counts( $network_id = null ) {
 ...
 * @since 3.7.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 * @since 6.0.0 This function is now a wrapper for update_user_counts().
 *
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function update_network_user_counts( $network_id = null ) {
	update_user_counts( $network_id );
}
 ...
 * Plugins can alter this criteria using the {@see 'is_large_network'} filter.
 *
 * @since 3.3.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 *
 ...
 */
function is_large_network( $using = 'sites', $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
		$network_id = get_current_network_id();
	}
	...
	if ( 'users' === $using ) {
		...
		$is_large_network = is_large_user_count( $network_id );
		 ...
		 */
		return apply_filters( 'is_large_network', $is_large_network, 'users', $count, $network_id );
	}

	$count = get_blog_count( $network_id );

	/** This filter is documented in includes/functions/ms-functions.php */
	return apply_filters( 'is_large_network', $count > 10000, 'sites', $count, $network_id );
}
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'admin', 'public', 'includes', 'api', and 'embed' were added
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $content );
	...
	sync_mail(
		...
		sprintf(
			...
			specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES )
		),
		$content
	);
 ...
 */
function network_admin_email_change_notification( $option_name, $new_email, $old_email, $network_id ) {
	$send = true;
...
###SITEURL###'
	...
	// Get network name.
	$network_name = specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES );
